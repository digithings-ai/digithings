...
**A task branch must be cut from a current base.** Module branches drift behind `develop` fast because we iterate on develop constantly — and a task branch cut from a stale module branch edits dead code. (Real incident, 2026-06-17: `module/digiquant` was ~2 months / ~400 commits behind, predating the `digiquant/src/digiquant/research → digiquant/src/digiquant/dashboard` migration; backend PRs cut from it touched files that no longer exist on develop.) The same hazard applied to the one-hop components until 2026-08-20: `scripts/worktree_task.sh` fetched the base branch only when no *local* ref of that name existed, and `refs/heads/develop` always exists, so `make task` handed out worktrees cut from whatever stale local `develop` you last pulled — one was measured 50 commits behind `origin/develop` (#2547).
...
release-please keeps targeting `develop`, so the version bump rides the
promotion into `main` — do not re-target it at `main`, or the squashed promotion
commit reads as a no-op and the changelog empties out.

**Every release also gets its own branch, cut from `main`.** Once the promotion
lands, cut `release/vX.Y.Z` from `main` at the release commit and tag it there —
one branch per released version. That branch is the version's **maintenance
line**, not a staging gate: features accumulate on `develop`, and by the time
the branch exists the version is already deployed. Patch releases continue the
chain (`release/vX.Y.(Z+1)` branched from `release/vX.Y.Z`); a fix on an older
line is cherry-picked onto `develop` so the next promotion carries it, and the
older line's version bump is never merged into `main`. Recipes:
