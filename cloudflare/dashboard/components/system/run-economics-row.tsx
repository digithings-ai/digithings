...
export function RunEconomicsRow({
  ...
  return (
    <div
      ...
      className="grid grid-cols-2 gap-px overflow-hidden border border-hair bg-hair md:grid-cols-4"
    >
      {metrics.map((metric) => (
        <div key={metric.label} className="min-w-0 bg-surface px-3 py-2.5 md:px-4 md:py-3">
          <span className="block font-mono text-[0.58rem] uppercase tracking-[0.1em] text-ink-mute">
            {metric.label}
          </span>
          <span className="mt-1 block truncate font-mono text-sm tabular-nums text-ink md:text-base">
            {metric.value}
          </span>
        </div>
      ))}
