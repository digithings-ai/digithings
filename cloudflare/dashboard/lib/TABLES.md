...
> (`cloudflare/digiweb/MIGRATION.md`, promotion playbook step 3 — the
> `lib/CHARTS.md` engine ruling is the in-app precedent).

## What the primitive offers

`SortableTable` (`cloudflare/digiweb/web/src/components/finance-composites/SortableTable.tsx`)
...
| Marks / unrealized | Open-book `positions.metrics_as_of` + `current_price` / market-API `GET /v1/market/closes` (#4013, API-backed; Supabase `price_history` close only when `NEXT_PUBLIC_MARKET_DATA_URL` is unset) | `metrics_as_of` or close date | Unrealized % vs avg entry | Fail closed without basis or mark | `marksUnstamped` when the open book is empty or any row lacks `metrics_as_of`. Chrome must not imply a refresh. |
...
- `cloudflare/dashboard/lib/performance-ssot.ts` — pure SSOT helpers
- `cloudflare/dashboard/lib/observability-queries.ts` — `getPerformanceBundle()` (Tearsheet + shared builder)
- `cloudflare/dashboard/app/page.tsx` — Brief persisted path + live-marks badge
- `cloudflare/dashboard/app/portfolio/performance/page.tsx` — Tearsheet consumer
