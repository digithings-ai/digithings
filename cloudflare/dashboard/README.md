...
Next.js 15 investment-intelligence dashboard for **digiquant** — research, portfolio, and execution in one operator surface (ADR-0026). Public path is `/dashboard/` (`basePath`); workspace folder is `cloudflare/dashboard` (npm package `dashboard`). Joins the root npm workspace and consumes the shared design system via
...
**CSP (REM-077):** security headers ship from `cloudflare/digiquant-web/public/_headers`,
...
digichat — see `cloudflare/digichat/.env.example`.
...
npm --workspace cloudflare/dashboard run check:static-export # verify server/client class boundaries
npm --workspace cloudflare/dashboard run lint
npm --workspace cloudflare/dashboard run test    # Vitest (lib/**/*.test.ts + components/**/*.test.tsx)
```
...
- Pipeline has three separate inspection surfaces: the topology explains process and
  ...
  ordered model/search/tool operations from `run_event_trace`. Soft-stamped
