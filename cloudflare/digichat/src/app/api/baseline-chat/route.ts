import {
  isLocalBaselinePreview,
  baselineUpstreamChatUrl,
  BASELINE_DEFAULT_UPSTREAM,
  BASELINE_UPSTREAM_USER_AGENT,
} from "@/lib/baseline-preview";
...
/**
 * Localhost assistant-ui baseline preview: proxy the live digithings.ai chat BFF.
...
export async function POST(req: Request) {
  ...
  const upstream = baselineUpstreamChatUrl();
  ...
  headers.set(
    "user-agent",
    req.headers.get("user-agent")?.trim() || BASELINE_UPSTREAM_USER_AGENT,
  );
  ...
  try {
    ...
    return jsonError(502, "baseline_upstream_failed", message);
  }
...
export function defaultBaselineUpstream(): string {
