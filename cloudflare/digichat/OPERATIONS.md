...
Apply new tables with `cd cloudflare/digichat && npm run db:migrate` (see [drizzle/0001_conversations.sql](drizzle/0001_conversations.sql), [drizzle/0002_quant_runs.sql](drizzle/0002_quant_runs.sql) for quant run storage).
...
3. **digichat env** — `cd cloudflare/digichat && cp -n .env.example .env.local`, then set at least:
...
```bash
cd cloudflare/digichat && DIGICHAT_DATABASE_URL=postgresql://digichat:digichat@127.0.0.1:5433/digichat npm run db:seed
...
- **`upstream_auth` / missing JWT:** If `DIGIKEY_URL` points at digikey, **`DIGIKEY_BFF_TOKEN` must match** the secret on the digikey process (same value in root `.env` / compose and `cloudflare/digichat/.env.local`). Restart digichat after changing env. See [../docs/LOCAL_STACK.md](../docs/LOCAL_STACK.md) for the full matrix. Alternatives: call the BFF with **`Authorization: Bearer dgk_live_…`**, or set **`DIGIGRAPH_UPSTREAM_API_KEY`** for a static upstream Bearer.
...
The old `cloudflare/website/` landing and `website/digichat/` zero-dependency demo were removed. Marketing chat is **`cloudflare/digithings-web` `/chat`**; digichat itself lives in **`cloudflare/digichat/`**.
