...
Before making any change to `cloudflare/digichat/`:

- [ ] Read `ARCHITECTURE.md` sections for the area you're touching (auth, chat route, conversations, ecosystem, DB schema)
- [ ] Run `npm run test` from `cloudflare/digichat/` — passes before and after
- [ ] Run `npm run lint` from `cloudflare/digichat/` — zero errors
