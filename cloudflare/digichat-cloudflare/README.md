...
| Legacy anonymous embed | `DIGICHAT_EMBED_ENABLED` is deliberately **unset** in `wrangler.toml` (OFF). A stock deploy refuses unregistered hosts. Opt in with `DIGICHAT_LEGACY_EMBED_ENABLED=1` only on a deployment **without** `DIGICHAT_EMBED_TENANTS`; with tenants configured the flag is ignored. |
...
# `"version"` must match cloudflare/digichat/package.json (baked into the image)
