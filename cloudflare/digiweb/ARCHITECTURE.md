...
screen imports compact `AuthCard` (`cloudflare/dashboard/components/login-screen.tsx`).
...
(a relative `../cloudflare/digiweb/web/...` import), the `cloudflare/digiweb/design/**`
...
```jsonc
{
  ...
  "source": "cloudflare/digiweb/reference",
...
```bash
node cloudflare/digiweb/scripts/build-manifest.mjs
```
...
`cloudflare/digiweb/brand/` — avatars from the favicon tile, OG cards and social
...
digithings.ai). See `cloudflare/digiweb/brand/README.md`.
...
(`test-web.yml`), gated on `cloudflare/digiweb/reference/**`: `npm run lint` and
...
because neither site builds it; `cloudflare/digiweb/reference/**` is watched by the
...
directory — only `cloudflare/digiweb/design/` sits in `score.py`'s skip list — but
`ci.yml`'s `score` path filter excludes `cloudflare/**` (#1310), so the lane does
not fire on a frontend-only PR. Note the distinction: the CI *filter* excludes
`cloudflare/**`, the *tool* does not.
