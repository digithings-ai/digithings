...
- **Charts.** DASHBOARD prices/candles/equity use TradingView **Lightweight
  ...
  `cloudflare/digiweb/CHARTS.md`). The full canvas
...
1. Create the component in `components/` (`"use client"` only if it needs
   ...
   (`cloudflare/digiweb/design/references/mine/index.html`) or the canon
   (`cloudflare/digiweb/design/spec/index.html`).
...
4. Verify from `cloudflare/digiweb/reference/`: `npm run typecheck` and `npm run lint`
