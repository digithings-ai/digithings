...
Everything here is DERIVED, so it cannot drift again:
  ...
  * the block cursor is one character cell — 0.6em wide, which is Geist Mono's
    ...
    cloudflare/digiweb/web/src/components/symbols/terminal-marks.tsx;
...
Usage:
    cloudflare/digiweb/brand/build-og.py           # write both cards
    cloudflare/digiweb/brand/build-og.py --check    # verify SVGs are in sync
"""
...
def card(font: TTFont, key: str) -> str:
  ...
  <!-- GENERATED by cloudflare/digiweb/brand/build-og.py — do not hand-edit.
