...
> dot (#1231). The shared CSS/JS **primitives below still exist** in `cloudflare/digiweb/design/`
...
Phase B) consumed directly by the v7 landings (`cloudflare/digithings-web`,
`cloudflare/digiquant-web`), plus `@digithings/web` (the React component layer —
...
### digithings.ai (`cloudflare/digithings-web/`)
...
### digiquant.io (`cloudflare/digiquant-web/`)
...
### twelve-x (`cloudflare/dashboard/components/twelve-x/`)
...
## 6. Primitives to build (shared `cloudflare/digiweb/design/`)
...
| `ScrollyFeatures` ✅ | Pinned section + progress rail + N slides — React hook `useScrollyFeatures` in `@digithings/web` (`cloudflare/digiweb/web/src/motion/scrolly.tsx`); live `PipelineScene` adoption deferred (see below) | Graphite |
...
> **ScrollyFeatures landed as a React hook, not vanilla `cloudflare/digiweb/design/`.** #1205's
> original spec ("shared module JS+CSS under `cloudflare/digiweb/design/`, built on
> `scroll-trigger.js`") predated the finding that the vanilla `cloudflare/digiweb/design/site/*.js`
...
| `cloudflare/digithings-web/components/landing/` | digithings landing components |
| `cloudflare/digiquant-web/components/landing/` | digiquant landing components |
| `cloudflare/dashboard/components/twelve-x/` | twelve-x research UI |
