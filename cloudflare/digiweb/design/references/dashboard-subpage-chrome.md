...
[`cloudflare/dashboard/components/subpage-tab-bar.tsx`](../../dashboard/components/subpage-tab-bar.tsx).
...
- Flat `--surface` panels (`.glass-card` is a flat panel, *not* glass), 1px `--hair`
  ...
  `cloudflare/dashboard/app/globals.css`.
...
note in `cloudflare/dashboard/app/globals.css` (#1216). dashboard has no `ARCHITECTURE.md`,
