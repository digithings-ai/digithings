...
- digithings.ai → `cloudflare/digithings-web` (Next.js static export)
- digiquant.io  → `cloudflare/digiquant-web` (+ `cloudflare/dashboard` at `/dashboard`)
- Shared design system → `cloudflare/digiweb/design` (tokens.css, site/site.css, components.css)
  and `cloudflare/digiweb/web` (`@digithings/web` — ThemeProvider, web-theme.css)
- LEGACY / not deployed: `cloudflare/digithings`, `cloudflare/digiquant` (old static HTML)
...
[`cloudflare/digiweb/design/references/`](../references/README.md) and
[`cloudflare/digiweb/design/EVOLUTION.md`](../EVOLUTION.md) — Graphite (scroll/motion),
...
Output dir: cloudflare/digiweb/design/demos/digiquant-landing/
...
Artifacts so far: cloudflare/digiweb/design/demos/digiquant-landing/01..04 (screenshotted, reviewed).
...
- Build artifacts in cloudflare/digiweb/design/demos/digiquant-landing/ (06-frontier.html exists; next 07).
...
DONE: (1) QR for digiquant.io — segno-generated scannable QR encoding https://digiquant.io,
  ...
  cloudflare/digiquant-web/public/favicon-qr.svg + -light.svg (replaced old). Phone-scan before prod.
...
- Tearsheet should look like the ACTUAL tearsheet (the PDF-export one in
  cloudflare/digiquant-web/components/tearsheet/). Model demo on real tearsheet-view layout.
...
## Next iterations
1. Read shared `cloudflare/digiweb/design/site/site.css` + `cloudflare/digiweb/web/styles/web-theme.css`
