...
## Context
We iterated a self-contained HTML prototype (`cloudflare/digiweb/design/demos/digiquant-landing/07-scroll-driven.html`) into a locked design direction for **digiquant.io**: a centered editorial "A quant hedge fund. In a box you own." hero over a mouse-following mesh-gradient, a scroll-pinned **dashboard** pipeline (research → portfolio → execution, real phases, one continuous horizontal track, execution "in development"), a **vertical tearsheet** strategy suite on the **real** BTC/ETH/SOL Slapper backtests, lowercase **digiquant** branding with the **QR** mark, and an open-source closing section. The real Next.js app (`cloudflare/digiquant-web`) still ships the old, rejected "headline-left + chart-right + marquee ticker + pricing" home. This change replaces the real app's design language with v7, across all pages.

Decisions (confirmed with user):
- **No live tape / no live market prices this round.** Ship the v7 design without any live data. (If revisited later, source = the project's Supabase `price_history` table — daily OHLCV, anon-readable, same client pattern as `cloudflare/dashboard/lib/supabase.ts`. Out of scope now.)
- **Whole-site port** (home + pipeline + strategies + tearsheet + subsystems).
- **All changes stay inside `cloudflare/digiquant-web/`.** Do NOT edit `@digithings/web` or `@digithings/design` — they are shared with **digithings.ai** and must not change. The design tokens there already match v7 exactly (`--accent #3DD6C4`, `--up`, `--down`, `--bg #0B0C0E`, Geist sans/mono), so v7 is expressible without touching shared CSS.
...
### New components — `cloudflare/digiquant-web/components/landing/`
...
### Pages — `cloudflare/digiquant-web/app/`
...
### CSS — `cloudflare/digiquant-web/app/globals.css`
...
- The working v7 reference: `cloudflare/digiweb/design/demos/digiquant-landing/07-scroll-driven.html` (port its CSS/JS into React).
...
## Verification
1. `npm install` in the worktree if needed, then `npm --workspace cloudflare/digiquant-web run dev` via the existing `digiquant-web` launch config (port 4011); preview_screenshot the home, `/pipeline`, `/strategies`, a `/strategies/<id>` tearsheet, and a `/subsystems/<id>` page at desktop + mobile; verify scroll-pinned pipeline advances research→portfolio→execution, the mesh follows the cursor, the strategy suite swaps tearsheets, and no console errors.
2. Confirm digithings.ai is untouched: `git status` shows no changes under `cloudflare/digiweb/web`, `cloudflare/digiweb/design`, or `cloudflare/digithings-web`.
3. `npm --workspace cloudflare/digiquant-web run build` (static export) succeeds; check `out/index.html`, `out/strategies/btc_slapper/index.html`, `out/subsystems/research/index.html` exist.
