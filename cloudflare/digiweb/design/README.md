...
lives in `cloudflare/digiweb/design/tokens.css` + `cloudflare/digiweb/design/components.css`,
...
- `cloudflare/digithings-web/` — digithings.ai (Next.js; imports `tokens.css` + `site/site.css` directly)
- `cloudflare/digiquant-web/` — digiquant.io (Next.js; imports `tokens.css` + `site/site.css` directly)
- `cloudflare/digichat/` — digithings.ai/chat (Next.js; workspace dep, token adoption tracked by #240)
- `cloudflare/dashboard/` — workspace dep only; token adoption deferred
...
`cloudflare/digiweb/design/tokens.css`.
...
Primitives live in `cloudflare/digiweb/design/components.css` and take their visual values
from tokens.
...
`cloudflare/digiweb/design/starfield.js`:
...
`cloudflare/digiweb/design/scroll-trigger.js`:
...
1. Vendor the tokens. Add a make target that copies `cloudflare/digiweb/design/tokens.css`
   into `digichat/src/app/tokens.css`:

   ```make
   sync-tokens:
   	cp cloudflare/digiweb/design/tokens.css cloudflare/digichat/src/app/tokens.css
   ```
...
Run any static server from `cloudflare/`:
