...
`site.css` is imported directly by `cloudflare/digithings-web/app/globals.css`
and `cloudflare/digiquant-web/app/globals.css` — the live Next.js marketing
...
(`.term*`/`.tl-*`, consumed by `cloudflare/digiweb/web/src/components/Terminal.tsx`),
...
```js
// Option A — vanilla pages: cloudflare/digiweb/design/scroll-trigger.js
...
// Option B — React: cloudflare/digiweb/web/src/motion/primitives.tsx's <Reveal>
...
`cloudflare/digiweb/design/smoke/index.html` has a vanilla-JS renderer.
