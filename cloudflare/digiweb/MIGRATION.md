...
in `ci.yml` on every PR/push (#1434) — it scans the whole `cloudflare/` tree via
...
Before writing new UI, check `cloudflare/digiweb/MANIFEST.json` (105 components,
...
- Utility missing from compiled CSS entirely → missing `@source`, or a stale
  Turbopack cache: `rm -rf cloudflare/<app>/.next` after switching branches.
