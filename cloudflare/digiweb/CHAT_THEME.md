...
- Do not adopt the legacy `ChatMarkdown` / `ChatThinking` / `ChatToolCall`
  exports for new work — they remain for the dashboard `SafeMarkdown` styling
  shell and digichat-ui (`MiniMarkdown`, activity view) only.
  `ChatMarkdownSource` is internal to `ChatMarkdown` and is not exported from
  the barrel (#3819)
