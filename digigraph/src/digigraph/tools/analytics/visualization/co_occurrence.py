...
def entity_co_occurrence(
    ...
    for c in entity_columns:
        if c not in df.columns:
            return {"error": f"Column {c!r} not found", "pairs": [], "image_path": None, "mermaid_source": None}
    ...
    if len(entity_columns) < 2:
        ...
        pairs = [{"value": r[0], "count": r[1]} for r in zip(counts[entity_columns[0]].to_list(), counts["count"].to_list())]
        return {"pairs": pairs, "image_path": None, "mermaid_source": None}
    a, b = entity_columns[0], entity_columns[1]
    pair_df = df.group_by([a, b]).agg(pl.len().alias("count")).filter(pl.col("count") >= min_count).sort("count", descending=True)
    pairs = [
