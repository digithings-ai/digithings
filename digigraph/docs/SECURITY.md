...
## MCP session tokens & checkpoints (#3794, #3969)

Operator MCP overlays may include short-lived OAuth/session `token` fields on
`WorkflowState.mcp_servers` for the active request. Those secrets are **in-request
only**: `digigraph.graph.mcp_checkpoint_redact.McpTokenRedactingCheckpointer`
strips secret-named keys before durable checkpointer `put` / `put_writes`.
Redaction is a recursive denylist, not a single `token` match — `authorization`,
`api_key`, `client_secret`, and comparable `*_token` / `*_secret` / `*_password`
names are removed wherever they appear, including nested `headers` / `auth`
sub-objects (#3969), so a future secret-bearing field cannot persist either.
Resume does not need persisted tokens — each HTTP turn overwrites `mcp_servers`
from the BFF header.
