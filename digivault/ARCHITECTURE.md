...
- **`digivault_search_notes` search precedence:**
  ...
  **`path_prefix` here is a filter, not an authorization boundary** — unlike
  ...
  requires, and the prefix arrives as an ordinary tool-call argument. Tenant
  isolation for search therefore still depends entirely on whoever calls this
  tool (digigraph) always attaching the right prefix — but as of #2240,
  digigraph does: `_handle_digivault_search` overwrites `path_prefix` from
  `ToolContext.vault_path_prefix` unconditionally, discarding whatever the
  model supplied, so the #2265 gap (digigraph's since-deleted
  `always_retrieve_tools` prefetch calling this tool with no prefix at all) is
  closed on digigraph's side. digivault's own half remains only to fail
  honestly (`ok=False`, above) rather than silently searching unscoped when no
  context prefix is available; it still cannot itself authenticate a
  `path_prefix` it did not choose — that guarantee lives entirely in
  digigraph's handler, not in this endpoint. The Supabase path is the same
