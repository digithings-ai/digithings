...
- **`digivault_search_notes` search precedence:**
  ...
  **Two independent enforcement layers now cover `path_prefix`, not one.**
  ...
  that tenant, is refused with `403`; a map that is set but unparseable or
  produces zero usable entries is `503` (`TenantCorpusMapError`), never
  silently treated as unset. The Supabase path is the same
