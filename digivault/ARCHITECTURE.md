...
- **`digivault_search_notes` search precedence:**
  ...
  **`path_prefix` is now also checked against the caller's authenticated
  ...
  no-op when that map is unset — single-tenant deployments (local dev, a
  self-hosted single-vault install) see no behavior change — and fails closed
  once it is set: a tenant absent from the map, or a `path_prefix` that
  doesn't match the map's entry for that tenant, is refused with `403`. The
