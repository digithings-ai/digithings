...
| `digivault/vault.py` | `Vault` / `FilesystemStore` — load a directory (or any store via `Vault.from_sources`), build the note index + link graph + backlinks + tag index; maintenance ops (`create_note`, `write_note(..., overwrite=True)` for idempotent upserts, `prune_children(parent_doc, keep_names, subdir)` for scoped docs_onboard convergence, `rename` with inbound-link rewrite, `set_frontmatter`, `reindex`, `lint`, `neighbors`). `create_note`/`write_note` and `lint` take an optional `scope` (blank = whole vault): a scoped collision check makes duplicate-stem handling prefix-neutral — `<scope>/<name>.md` is creatable while a same-stem note lives under another prefix, and `lint(scope=…)` reports a `duplicate_note` only when two or more colliding files are inside the scope, naming only in-scope paths (#4256). `FilesystemStore` is the explicit filesystem `VaultStore` name (#1142); `Vault` remains the public type. |
...
| `digivault/tool_dispatch.py` | **Canonical vault tool routing (#1188).** Owns every digivault tool name (`DISPATCH_TOOL_NAMES`), the vault-local handler table (`VAULT_HANDLERS` for tag / backlinks / lint / create-note), and MCP registration (`register_mcp_tools`). `mcp_server` and `server.orchestrator_invoke` both route vault-local tools through `dispatch_vault_tool` — no duplicate handler tables. Runtime-only tools (`digivault_search_notes`, `digivault_get_note`) claim into the same table via `register_runtime_handler` from `server.py` so `dispatch_tool_names()` equals the full canonical set once the HTTP app is loaded. MCP discovery (`mcp_tool_names()`) advertises the MCP surface name of every vault-local handler (`search_tag`, … — digigraph prefixes the server id to get `digivault_search_tag`) plus the MCP-registered `search_notes` runtime tool (required `path_prefix`). Vault-local handlers honour an optional `path_prefix` (the operator `setup.path_prefix` digigraph injects): reads and `create_note` are scoped beneath that subdirectory, with `../`/leading-dot components refused; blank keeps whole-vault behaviour. Duplicate-stem handling is scope-aware: `lint` scopes inside `Vault.lint(scope=…)` (a post-hoc filter of a whole-vault report leaked the colliding corpus's path) and `create_note` passes the prefix as `scope`, so a stem living only outside the prefix neither blocks the write nor leaks its existence (#4256 — existence oracle). |
...
| `digivault/mcp_server.py` | `python -m digivault.mcp_server [--host/--port/--stdio]` — thin transport wrapper; tools register exclusively via `tool_dispatch.register_mcp_tools` (streamable HTTP, default `127.0.0.1:8769`). Dedicated image `digivault/Dockerfile.mcp` (repo-root context, `[service]` extra). |
...
- **Filesystem note routes are tenant-scoped (#3915).** `_open_scoped_vault(request)`
  narrows the shared root to the caller's corpus when `DIGI_TENANT_CORPUS_MAP` is
  configured: every filesystem route (`GET /v1/notes`, `/v1/notes/{name}`,
  `/v1/notes/{name}/backlinks`, `/v1/tags/{tag}`, `/v1/lint`, and the `POST`/`PATCH`
  mutations) opens `Vault(<DIGIVAULT_ROOT>/<tenant prefix>)` — the filesystem analogue
  of D1's one-database-per-corpus isolation, so writes cannot escape the tenant
  subtree either because it is the vault root. `mapped_tenant_path_prefix` fails
  closed (403 for an unmapped tenant, 503 for a map set but unusable); a prefix that
  would resolve outside `DIGIVAULT_ROOT` is refused with 503. The map genuinely unset
  is unchanged — `_open_vault()` over the shared root — so every single-tenant
  deployment (local dev, a self-hosted single vault) behaves exactly as before. The
  vault-local orchestrator tools (lint/tag/backlinks/create_note) route through the
  same helper. Before this, these routes carried only `digivault:read`/`:write` and
  opened the shared root unscoped; the D1 by-path/search family was the only
  tenant-bound surface.
...
- **MCP hosting (stack slot :8769):** dedicated `digivault/Dockerfile.mcp` image
  (`digi-digivault-mcp`, repo-root context, `digivault[service]`), compose profile
  `digivault-mcp` (loopback `127.0.0.1:8769:8769`, `DIGIVAULT_ROOT`
  ...
  orchestrator-only.
  Vault-local tools carry no JWT check, so the MCP sidecar sets no `DIGIKEY_*` —
  treat a wider bind like a network API with gateway auth.
...
- **Canonical tool dispatch (#1188).** Vault MCP / orchestrator tool names and
  ...
  MCP `search_tag` (digigraph sees it as `digivault_search_tag`) projects a
  slim JSON **array** of `{name, title, rel_path}` (pre-#3041 contract);
  orchestrator invoke keeps the shared handler’s `{"notes": [...]}` envelope.
