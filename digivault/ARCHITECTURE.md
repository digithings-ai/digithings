...
- **MCP hosting (stack slot :8769):** dedicated `digivault/Dockerfile.mcp` image
  ...
  through the key-gated `/_stack/mcp/digivault/*` edge route). MCP exposes
  `search_notes` (required `path_prefix`; D1 -> local -> Supabase), tag search,
  backlinks, and lint over the vault; `create_note` registers only when
  `DIGIVAULT_MCP_WRITE=1`, and `digivault_get_note` stays orchestrator-only.
