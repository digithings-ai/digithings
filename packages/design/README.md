...
lives in `packages/design/tokens.css` + `packages/design/components.css`,
...
- `apps/digithings-web/` — digithings.ai (Next.js; imports `tokens.css` + `site/site.css` directly)
- `apps/digiquant-web/` — digiquant.io (Next.js; imports `tokens.css` + `site/site.css` directly)
- `apps/digichat/` — digithings.ai/chat (Next.js; workspace dep, token adoption tracked by #240)
- `apps/dashboard/` — token adopter: imports `tokens.css`, the `@digithings/ui` `web-theme.css` bridge and ~15 kit sheets
...
Primitives live in `packages/design/components.css` and take their visual values
from tokens.
...
`packages/design/scroll-trigger.js`:
...
1. Vendor the tokens. Add a make target that copies `packages/design/tokens.css`
   into `digichat/src/app/tokens.css`:

   ```make
   sync-tokens:
   	cp packages/design/tokens.css apps/digichat/src/app/tokens.css
   ```
...
Run any static server from `packages/design/`:
