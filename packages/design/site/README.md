...
`site.css` is imported directly by `apps/digithings-web/app/globals.css`
and `apps/digiquant-web/app/globals.css` — the live Next.js marketing
...
(`.term*`/`.tl-*`, consumed by `packages/ui/src/components/Terminal.tsx`),
...
reveal are React components in `@digithings/ui` (`chrome.tsx`, `DigiNav.tsx`
...
`@digithings/ui`, which reads/writes the same key.
...
```js
// Option A — vanilla pages: packages/design/scroll-trigger.js
...
// Option B — React: packages/ui/src/motion/primitives.tsx's <Reveal>
...
`packages/design/smoke/index.html` has a vanilla-JS renderer.
...
**Copy variants** (shown in `packages/design/smoke/index.html`):
