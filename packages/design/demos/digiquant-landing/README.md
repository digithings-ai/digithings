...
`apps/digiquant-web` app. Nothing here ships; it's the reference to port from.
...
- A **vertical tearsheet strategy suite** on the **real** BTC/ETH/SOL Slapper backtests
  (data from `apps/digiquant-web/public/strategies/index.json`); sticky tearsheet on the
...
3. Port `07-scroll-driven.html`'s CSS/JS into React client components under
   `apps/digiquant-web/components/landing/` and rewrite the pages. Reuse the existing
