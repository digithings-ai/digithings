...
- digithings.ai → `apps/digithings-web` (Next.js static export)
- digiquant.io  → `apps/digiquant-web` (+ `apps/dashboard` at `/dashboard`)
- Shared design system → `packages/design` (tokens.css, site/site.css, components.css)
  and `packages/ui` (`@digithings/ui` — ThemeProvider, web-theme.css)
...
- Worktree needs `npm install` once (no node_modules → @digithings/ui, geist fail to resolve).
...
[`packages/design/references/`](../references/README.md) and
[`packages/design/EVOLUTION.md`](../EVOLUTION.md) — Graphite (scroll/motion),
...
Output dir: packages/design/demos/digiquant-landing/
...
Artifacts so far: packages/design/demos/digiquant-landing/01..04 (screenshotted, reviewed).
...
- Build artifacts in packages/design/demos/digiquant-landing/ (06-frontier.html exists; next 07).
...
DONE: (1) QR for digiquant.io — segno-generated scannable QR encoding https://digiquant.io,
  ...
  apps/digiquant-web/public/favicon-qr.svg + -light.svg (replaced old). Phone-scan before prod.
...
- Tearsheet should look like the ACTUAL tearsheet (the PDF-export one in
  apps/digiquant-web/components/tearsheet/). Model demo on real tearsheet-view layout.
...
## Next iterations
1. Read shared `packages/design/site/site.css` + `packages/ui/styles/web-theme.css`
