...
Product Thread copies live in `@digithings/ui/chat/thread` (gallery
...
Style pin: **base-nova**. From `apps/digichat/` (or the gallery app with the
same registry):
...
| Kind | Meaning |
| ---- | ------- |
| **in gallery** | Fixture `/chatbot` page mounts `@digithings/ui/chat/thread` (not a second copy) |
...
- digichat deploy `features` in `apps/digichat/src/lib/deploy-config/schema.ts`
