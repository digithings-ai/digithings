        ...
        >
          ...
          <ThreadPrimitive.ViewportFooter
            className={cn(
              ...
              // Single-route plan: sticky in all states (the gallery Thread never
              // conditioned on isEmpty either) — empty footers pin to the bottom.
...
const Composer: FC<{ autoFocus: boolean }> = ({ autoFocus }) => {
  // Embed free-tier / BYOK hold: StockSendGateProvider supplies onSubmit that
  // preventDefaults before composer.send() so POST /api/chat never fires.
            ...
            switch (part.type) {
              ...
              case "group-tool":
                ...
                return (
                  <ToolGroupRoot
                    variant="ghost"
                    defaultOpen={toolCallsUi.defaultOpen}
                    {...(toolCallsUi.locked
              ...
              case "group-reasoning": {
                ...
                return (
                  <ReasoningRoot
                    streaming={running}
                    defaultOpen={reasoningUi.defaultOpen}
                    {...(reasoningUi.locked
                      ? { open: true, onOpenChange: () => {} }
                      : {})}
                  >
...
const UserMessage: FC = () => {
  const { userAlign } = useDeployUi();
  const alignEnd = userAlign === "right";
  return (
    <MessagePrimitive.Root
      data-slot="aui_user-message-root"
      data-user-align={userAlign}
      className={cn(
    ...
    >
      ...
      <div
        className={cn(
          ...
          alignEnd ? "col-start-2" : "col-start-1",
        )}
      >
