...
- Two live frontend deployments consume digiweb's packages by name: `@digithings/design` (tokens) and `@digithings/ui` (shared React components) — not by on-disk path, so digiweb can move without breaking consumers.
