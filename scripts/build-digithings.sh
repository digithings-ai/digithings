...
# digithings.ai is now a Next.js static-export app (apps/digithings-web);
...
FRAME_SRC="$(node --input-type=module -e '
  import { frameSrcForCsp } from "./apps/digithings-web/lib/security-headers.mjs";
...
# NOT inside the static output dir. Mirror from apps/digithings-web/functions/
...
if [ -d apps/digithings-web/functions ] && [ -n "$(find apps/digithings-web/functions -type f 2>/dev/null | head -1)" ]; then
  ...
  echo "ERROR: expected apps/digithings-web/functions (digivault /api/chat)" >&2
  exit 1
fi
