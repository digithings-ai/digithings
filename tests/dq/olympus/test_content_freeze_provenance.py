"""Content-freeze detection and provenance (#1749, #1750, #1751).
...
class TestFreshnessSurface:
    ...
    def test_marker_without_a_date_is_frozen_not_today(self) -> None:
        ...
        body = {**_HEALTHCARE_BODY, UNCHANGED_FLAG_KEY: True}
        fresh = _slot_freshness(self._payload(body))
        assert fresh.source == "frozen"
        assert fresh.as_of == "2026-07-31"

    def test_the_read_path_validator_accepts_frozen(self) -> None:
        """The hard coupling. ``atlas.snapshot.SegmentFreshness`` is ``extra="forbid"`` and
        validates rows on READ, so a value the writer emits but this ``Literal`` omits is a
        ValidationError on every later read of the row — not a soft ignore. Both copies of the
        model have to widen in the same change, and this is the test that says so."""
        row = ReadPathFreshness.model_validate({"source": "frozen", "as_of": "2026-07-28"})
        assert row.source == "frozen"

    def test_the_read_path_validator_still_accepts_the_old_values(self) -> None:
...
class TestFreezeTelemetry:
    ...
    def test_breakdown_reports_count_and_freeze_dates(self) -> None:
        state = AtlasResearchState(
            ...
            content_freezes={"sector-healthcare": "2026-07-28", "sector-materials": "2026-07-21"},
        )
...
class TestPublishedDeltaRow:
    ...
    def test_a_real_edit_records_content_changed_true_and_no_freeze_date(self) -> None:
        result = EditSegmentResult(slot=None, errors=[], delta=_patch(_NOOP_OPS))
        row = _delta_row(result)
