...
class TestResolveMaturedOutcomes:
    ...
    def test_repeating_decimal_quantized_to_return_fraction(self) -> None:
        # Regression for 2026-09-05 olympus-daily double-fail:
        # (mat-ref)/ref yields 29-digit repeating decimals, violating
        # ReturnFraction(max_digits=16, decimal_places=8) before the fix.
        assessment = _assessment(ref_price="66.71")
        mat = date(2026, 8, 13)
        client = OutcomesFake(
            canned_reads={
                "price_history": [
                    {"ticker": "AAPL", "date": mat.isoformat(), "close": "68.71"},
                ],
            }
        )
        _seed_assessment(client, assessment)
        ...
        assert result.resolved == 1
        row = client.store[fo.OUTCOMES][0]
        realized = Decimal(row["realized_return"])
        residual = Decimal(row["signed_residual"])
        mean = Decimal(row["forecast_mean_return"])
        for value in (realized, residual, mean):
    ...
    def test_quantize_matches_prod_failure_decimals(self) -> None:
        ...
        assert fo._quantize_return_fraction(Decimal("0.02998405466970387243735763098")) == Decimal(
            "0.02998405"
        )
        assert fo._quantize_return_fraction(Decimal("0.02698405466970387243735763098")) == Decimal(
            "0.02698405"
        )

    def test_unbuildable_outcome_stays_pending_without_raising(
        ...
        monkeypatch.setattr(fo, "_build_resolved_outcome", _boom)
        result = fo.resolve_matured_forecast_outcomes(
            client=client,
            run_date=RUN_DATE,
            knowledge_cutoff_at=CUTOFF,
