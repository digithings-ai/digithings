"""Contract tests for migration 082, cost/liquidity evidence registry (#2709)."""
...
MIGRATION_PATH = MIGRATIONS_DIR / "082_olympus_cost_liquidity.sql"

TABLES = (
    "olympus_liquidity_snapshots",
    "olympus_action_cost_estimates",
    "olympus_action_cost_outcomes",
)
...
def test_migration_is_the_only_082() -> None:
    assert sorted(MIGRATIONS_DIR.glob("082_*.sql")) == [MIGRATION_PATH]


def test_migration_follows_081() -> None:
    assert (MIGRATIONS_DIR / "081_olympus_risk_policy_snapshots.sql").is_file()
...
def test_estimate_fk_to_snapshot(sql: str) -> None:
    body = _table_body(sql, "olympus_action_cost_estimates")
    assert re.search(
        r"FOREIGN KEY\s*\(liquidity_snapshot_id\)\s+REFERENCES\s+public\.olympus_liquidity_snapshots",
        body,
        re.I,
    )


def test_outcome_fk_to_estimate(sql: str) -> None:
    body = _table_body(sql, "olympus_action_cost_outcomes")
    assert re.search(
        r"FOREIGN KEY\s*\(estimate_id\)\s+REFERENCES\s+public\.olympus_action_cost_estimates",
        body,
        re.I,
    )
...
def test_append_only_triggers(sql: str, table: str) -> None:
    assert "reject_olympus_cost_liquidity_mutation" in sql
