...
class TestRunBacktest:
    """POST /run_backtest. Returns 200 when Nautilus available, 503 when not."""

    def test_returns_200_with_valid_body_when_nautilus_available(self, client: TestClient) -> None:
    ...
    def test_returns_503_when_nautilus_unavailable(self, client: TestClient) -> None:
        with patch("digiquant.backtest.run_nautilus_backtest", return_value=None):
            r = client.post("/run_backtest", json=SAMPLE_BACKTEST_PAYLOAD)
            assert r.status_code == 503
            assert "nautilus" in r.json().get("detail", "").lower()

    def test_accepts_default_body_when_nautilus_available(self, client: TestClient) -> None:
        pytest.importorskip("nautilus_trader")
        r = client.post("/run_backtest", json={})
        assert r.status_code == 200
        assert r.json().get("run_id", "").startswith("nautilus-")

    def test_validation_rejects_invalid_types(self, client: TestClient) -> None:
        r = client.post("/run_backtest", json={"strategy_name": 123, "symbols": "not-a-list"})
...
class TestRunOptimize:
    """POST /run_optimize. Returns 200 when Nautilus available, 503 when not."""

    def test_returns_200_with_valid_body_when_nautilus_available(self, client: TestClient) -> None:
        pytest.importorskip("nautilus_trader")
        r = client.post("/run_optimize", json={"strategy_name": "ema_cross", "symbols": ["AAPL"]})
        assert r.status_code == 200
        data = r.json()
    ...
    def test_accepts_default_body_when_nautilus_available(self, client: TestClient) -> None:
        pytest.importorskip("nautilus_trader")
        r = client.post("/run_optimize", json={})
        assert r.status_code == 200
        assert r.json().get("best_backtest") is not None or r.json().get("num_evaluations") >= 1
...
class TestRunPipeline:
    """POST /run_pipeline. Returns 200 when Nautilus available, 503 when not."""

    def test_returns_200_with_backtest_optimize_export_when_nautilus_available(self, client: TestClient) -> None:
        pytest.importorskip("nautilus_trader")
        r = client.post(
            "/run_pipeline",
            json={"strategy_name": "ema_cross", "symbols": ["AAPL", "MSFT"]},
        )
        assert r.status_code == 200
        data = r.json()
    ...
    def test_returns_503_when_nautilus_unavailable(self, client: TestClient) -> None:
        with patch("digiquant.backtest.run_nautilus_backtest", return_value=None):
            r = client.post(
                "/run_pipeline",
                json={"strategy_name": "ema_cross", "symbols": ["AAPL"]},
            )
            assert r.status_code == 503
