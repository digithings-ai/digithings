...
- `s5_category_company.json` — live EXA `/search` response (type=auto, category=company,
  ...
  LinkedIn media URLs it carried embed signed query tokens. Live re-validation is the
  Phase D Task 8 live record.
...
- `fx_ecb_snapshot.json` — ECB euro foreign exchange reference rates fetched from the
  ...
  funding normalization; refreshed alongside the Task 8 live record.
