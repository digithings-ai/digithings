"""Pin wave 3: the dashboard workspace lives at apps/dashboard.
...
PKG = REPO_ROOT / "apps" / "dashboard" / "package.json"
...
def test_workspace_folder_is_frontend_dashboard() -> None:
    assert PKG.is_file(), "apps/dashboard/package.json must exist"
...
def test_build_copies_from_frontend_dashboard() -> None:
    ...
    assert "cp -r apps/dashboard/out/. dist/dashboard/" in text
...
def test_canon_census_app_is_dashboard_folder() -> None:
    ...
    assert "apps/dashboard/lib/chart-colors.ts" in text
    assert "cloudflare/olympus/" not in text


def test_dashboard_does_not_ship_olympus_public_env_keys() -> None:
    env = (REPO_ROOT / "apps" / "dashboard" / ".env.local.example").read_text(encoding="utf-8")
...
def test_dashboard_icons_and_theme_keys_are_not_olympus() -> None:
    ...
    theme = (REPO_ROOT / "apps" / "dashboard" / "components" / "theme-provider.tsx").read_text(
        encoding="utf-8"
    )
    icons = REPO_ROOT / "apps" / "dashboard" / "public" / "icons"
