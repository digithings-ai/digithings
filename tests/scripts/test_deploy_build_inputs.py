...
fell behind it: `apps/digithings-web` took a dependency on `@digithings/digichat-ui`
in #1384, and the digithings.ai filter was never updated. PR #1490 then edited
`packages/digichat-ui/src/DigiChatSession.tsx` — a file that site renders — alongside
...
def _workspace_packages() -> dict[str, tuple[str, set[str]]]:
    ...
    list, so `packages/brand` and `scripts` — directories with no
...
def test_every_workspace_manifest_is_watched_by_some_filter() -> None:
    ...
    reached by some filter; ``apps/reference/package.json`` was in none at
    all, and both deploy checks now name it.

    Naming it fixed the instance. This fixes the class: the set of workspaces moves —
    ``apps/reference`` was itself scaffolded and then moved into place — and
    ...
    ``apps/digichat``'s second cover is ``ruff_and_scripts``, which installs nothing.
...
def test_digichat_ui_is_a_digithings_build_input_but_not_a_digiquant_one() -> None:
    ...
    assert "packages/digichat-ui" in digithings
    assert "packages/digichat-ui" not in digiquant
    ...
    assert "apps/digichat" not in digithings | digiquant
