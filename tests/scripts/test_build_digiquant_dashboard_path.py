...
def test_build_copies_dashboard_export_only_to_dist_dashboard() -> None:
    ...
    assert "cp -r apps/dashboard/out/. dist/olympus/" not in text
...
def test_settings_kicker_is_dashboard_not_olympus() -> None:
    page = (REPO_ROOT / "apps" / "dashboard" / "app" / "settings" / "page.tsx").read_text(
        encoding="utf-8"
    )
...
def test_public_app_urls_ok_pins_dashboard_not_olympus() -> None:
    ...
    text = (
        REPO_ROOT / "digiquant" / "src" / "digiquant" / "execution" / "staging_e2e.py"
