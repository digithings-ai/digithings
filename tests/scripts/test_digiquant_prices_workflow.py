...
def test_no_quotes_writer_remains() -> None:
    """Both fetch-quotes writers are retired (#4053): intraday price_history and
    the paused eod-macro sector refresh. Same-day opens now live-fetch."""
    assert not [s for s in _steps() if "fetch-quotes" in str(s.get("run", ""))]


def test_any_remaining_supabase_market_write_is_run_writers_gated() -> None:
    """The only Supabase market write left is the paused eod-macro fetch-macro.
