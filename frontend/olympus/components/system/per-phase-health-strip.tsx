          ...
          return (
            <div key={p.phase} className="flex items-center gap-3 text-xs">
              ...
              <span className="flex h-2 flex-1 overflow-hidden rounded-full bg-term-bg" aria-hidden>
                {/* Status chrome, not P&L (F5) — ok=accent; carried=soft warn; failed=full warn. */}
                <span className="bg-accent" style={{ width: `${(p.ok / total) * 100}%` }} />
                <span className="bg-warn/60" style={{ width: `${(p.carried / total) * 100}%` }} />
                <span className="bg-warn" style={{ width: `${(p.failed / total) * 100}%` }} />
              </span>
