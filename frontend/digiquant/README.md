...
| `strategies.html` | Strategy-library index — one card per published tearsheet  |
| `strategies.js` | Library module — fetches `strategies/index.json`, renders cards |
| `tearsheet.html` | Standalone strategy tearsheet shell (`?s=<strategy>`)       |
| `tearsheet.js` | Tearsheet renderer — KPIs, breakdown, charts, trade log, PDF    |
| `tearsheet-charts.js` | Dependency-free SVG charts (equity, drawdown, P&L; log/symlog) |
| `tearsheet.css` | Tearsheet + library styles (semantic [data-theme] tokens, print) |
| `strategies/` | Published `TearsheetData` JSON + `index.json` manifest          |
...
The tearsheet pages use the redesign foundation (`../design/site/site.css` +
`[data-theme]` tokens, Geist fonts, light/dark toggle). The `strategies/*.json`
files are generated by
[`scripts/validation/make_tearsheets.py`](../../scripts/validation/make_tearsheets.py)
(unified schema in `digiquant.tearsheet_data`) and committed as the published
artifacts the renderer fetches. "Download PDF" prints full-bleed, theme-matched.
