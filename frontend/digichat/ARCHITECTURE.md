...
| `models.default` / `models.available` / `models.allowPicker` | strings + bool | BFF allowlists `available` on `POST /api/chat` (`X-Digi-Model`); empty `available` = no restriction. Product embed (`digithings-ai-embed.yaml`) publishes DeepSeek V4 Flash (default) plus OSS at that price and OpenRouter `:free` ids (#3777). Not gemini-3.1-flash-lite or gpt-5.6-luna. |
