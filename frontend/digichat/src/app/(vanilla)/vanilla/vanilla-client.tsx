...
/**
 * Official assistant-ui templates (the 11 catalog ids).
 * Transport is the localhost vanilla BFF, which proxies production
 * digithings.ai/api/chat (Cloudflare stack).
 *
 * `?skin=base|chatgpt|claude|grok|gemini|perplexity|react-ink|expo-react-native|base-assistant-ui|webpage-assistant|product-page-assistant`
 * `?theme=dark|light`
 */
export function VanillaClient() {
  return (
...
function VanillaClientInner() {
  ...
  const hrefFor = (nextSkin: string, nextTheme: string) => {
    ...
    return query ? `/vanilla?${query}` : "/vanilla";
  };
