...
Style pin: **base-nova**. From `frontend/digichat/` (or the gallery app with the
same registry):
...
| [`composer-trigger-popover`](https://www.assistant-ui.com/elements/composer-trigger-popover) | A character-triggered picker for mentions, slash commands, and nested composer actions. | `npx shadcn add @assistant-ui/composer-trigger-popover` | chrome-driven | shared primitive under slash/mentions |
...
- digichat deploy `features` in `frontend/digichat/src/lib/deploy-config/schema.ts`
