...
**Body Font:** Geist (with system-ui, -apple-system, sans-serif fallback)
**Label/Mono Font:** Geist Mono (with JetBrains Mono, ui-monospace, monospace fallback)

**Character:** A precise, cockpit-flavored mono voice does almost everything — chrome, labels, buttons, data, tables, terminal text — while a single quiet serif appears only where the product makes a genuine human claim: a marketing headline, a markdown heading inside a streamed answer, an account holder's name. The two voices never compete inside one component.

### Hierarchy
- **Display** (400, `clamp(2rem, 5vw, 3.4rem)`, 1.1 line-height, -0.02em tracking): hero claims, section titles, markdown `h1`–`h6` (all six levels flatten to this one size so a stray heading in a streamed chat answer never out-ranks the transcript's own hierarchy). Never bolded.
- **Body** (400, ~1rem, 1.65 line-height): reading copy, capped at 65–75ch measure.
...
Two radius families coexist by role, not by size preference: **pill** (`999px`) means "you can click or select this" — every button, badge, toggle, chip, tab, and tooltip trigger is a pill — while a **rectangular radius scale** (`8px` / `12px` / `16px`) means "this is a container" — cards, panels, dropdown panes, form fields step up through that scale as they grow from a compact control to a generous card. Device mockups (the phone frame) deliberately break from the token scale with bespoke bezel radii, because they are simulating hardware, not a UI surface.

Borders are uniformly 1px hairline; the only thicker strokes are decorative chevron corner-strokes and a slider thumb's focus ring. There is no visible border-radius scale drift between products: 8/12/16px is the one rectangular scale a new surface should reach for.

**The Shape-Carries-Meaning Rule.** Do not use a pill for a container or a rectangular radius for a clickable control — the distinction is load-bearing across the whole system, not a stylistic accident.

## Components

**Instrumentation, not decoration.** Every control in the system is built from the same restrained kit: pill shape for anything actionable, hairline rectangles for containers, mono type for all chrome, and color spent only where it signals state — a selection, a focus, a gain, a loss, a live process. Nothing is styled to look busy; busyness would obscure the one thing that actually changed.
...
- **Disabled:** `opacity: 0.4`, `cursor: not-allowed` — defined for the primary variant; other variants should get the same treatment when built (the reference implementation currently omits it for ghost/quiet/danger — don't repeat that gap).
- **Loading:** an inline 11px two-tone spinner, killed under `prefers-reduced-motion`.

### Chips / Tabs / Segmented Controls (the selection family)
- **Style:** pill or bordered-strip, hairline border, `ink-mute`/`ink-soft` idle text.
- **Selected state:** text promotes to `ink`, border and background both tint toward the accent via `color-mix(in srgb, var(--accent) 45–60%, var(--hair or --surface))` — a soft wash, never a hard fill, so the accent reads as "chosen" rather than "primary action."
...
### Cards / Containers
- **Corner Style:** `12px` (`--r-md`) standard; `8px` for compact controls, `16px` for the largest panels.
- **Background:** flat `--surface`, no gradient, no glass.
- **Shadow Strategy:** none (see Elevation & Depth) — depth is the hairline border alone.
...
### Inputs / Fields
- **Style:** `8px` radius, hairline border, `--bg`-colored fill (reads as canvas cut into the surrounding surface card), mono type regardless of field type.
...
### Navigation
NavShell settles (gains a blurred hairline backdrop) after 8px of scroll and either yields off-screen past 180px of downward scroll (returning immediately on any upward scroll) or, on app-like surfaces, reveals within a top hot-zone and releases once the pointer clears it. A dropdown menu pins the bar open. Below `880px` the inline strip becomes a hamburger opening a full-height sheet, its link stack set in the display serif — the one editorial-serif moment inside otherwise-mono chrome. The wordmark's suffix (the part of "digithings" after "digi") is always the accent-colored emphasis, everywhere the brand appears.
...
- **Do** treat Instrument Serif as the production display font; treat any Fraunces sighting as reference-app furniture, not a target to copy.
