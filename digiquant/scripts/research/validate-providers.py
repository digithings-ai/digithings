...
Fail-fast house (Cheaper Inference): there are no OpenRouter-specific LLM
checks — provider errors surface from the real run instead of being probed
(and potentially masked) here.

Checks (in order):
  1. Required env vars are present (Supabase + house Cheaper Inference key)
  2. Supabase is reachable and daily_snapshots has a prior baseline row
     (required for --auto-baseline on delta runs)
  3. Graph compiles cleanly — dry-run for both baseline and delta

Usage:
  python scripts/validate-providers.py          # from repo root or research dir
  python scripts/validate-providers.py --skip-db   # env-var check only
  python scripts/validate-providers.py --skip-dry-run  # env-var + DB check only
...
def _configure_preflight_environment() -> None:
    """Apply production house routing and bound digillm before the first LLM import."""
...
def check_supabase() -> bool:
    print(_bold("\n2. Supabase connectivity + baseline row"))
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Tip: load your local env before running:
              ...
              export CHEAPERINFERENCE_API_KEY=...
        """),
    )
