...
across the validated baseline plus all 4 Bitview on-chain series.

Per Chris's explicit redirection ("add as many indicators as possible, then
do one weighting/reweighting pass -- an optimization for the weights all at
once"): rather than reweighting after each on-chain indicator individually,
this runs ONE ``stage_a.optimize_stage_a_weights_combined`` pass across the
validated baseline (``power_law=1.0, m2=0.5, dxy=0.5`` --
``RESEARCH_STATE.md``'s "current best validated candidate") plus
``onchain_mvrv``, ``onchain_asopr``, ``onchain_puell``, ``onchain_rhodl`` --
all 4 of which cleared the Stage 1 solo-validation noise-baseline gate (see
``run_onchain_solo_search.py``).

Deliberately scoped to just the validated baseline + on-chain series, not
the full EXTRA_INDICATOR_NAMES catalog: the price-oscillator indicators
(weekly_rsi/macd, sma_band, rs_eth, monthly/weekly_monthly variants) are a
separate, still-open research thread (see RESEARCH_STATE.md's "Cursor
Agent" open question) and mixing them in here would conflate two different
lines of work.

Per the Phase B playbook step 3: a candidate is only worth keeping if it
earns a weight *strictly above* the 0.1 floor in the winning mix -- a
candidate pinned at the floor is contributing no signal beyond what the
floor forces.

Grid: a single 5-point pass (0.1, 0.325, 0.55, 0.775, 1.0) across all 7
dimensions (power_law + 6 extras) -- 5**7 = 78,125 evaluations, the same
order of magnitude as the ~390K-evaluation 5-point/8-dimension run already
completed for the surviving-7 price-oscillator mix (RESEARCH_STATE.md).
``optimize_stage_a_weights_combined`` takes one shared grid across every
search dimension, not a per-indicator grid, so a coarse-then-refine pass
(as used for the full 11-indicator recalibration, where a 5-point grid over
11 dims is too large to brute-force directly) isn't needed at only 7
dimensions -- one 5-point pass already gives 0.225-step resolution.
...
Usage:
    python scripts/run_onchain_expanded_reweight.py
"""
...
SEARCH_NAMES = ("m2", "dxy", "onchain_mvrv", "onchain_asopr", "onchain_puell", "onchain_rhodl")
...
def run(data_path: Path = DEFAULT_DATA_PATH) -> None:
    ...
    print(
        f"{'reweighted (expanded set)':<32} {result.score.long.objective:>8.2f} "
