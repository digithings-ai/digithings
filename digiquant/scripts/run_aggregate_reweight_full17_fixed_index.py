#!/usr/bin/env python3
"""Aggregate reweight (Stage 4-equivalent) over the full 17-indicator survivor
list -- re-run of ``run_aggregate_reweight_full17.py`` with the extras index
built using each indicator's real Stage 1 period instead of the shared
``DEFAULT_ROLLING_WINDOW`` (90) default.

Round 1 (``run_aggregate_reweight_full17.py``) built its ``extra_z`` via
``load_sdca_extra_z(..., extra_windows=None)`` -- every macro/on-chain extra
(m2, dxy, the four on-chain series, fear_greed) got window=90 regardless of
its own period-search result. This run instead passes the newly-plumbed
``extra_windows`` override (``load_sdca_extra_z`` -> ``extra_z_vectors`` ->
``build_extra_indicators``, see indicator_catalog.py) with each indicator's
real Stage 1 window:

    dxy=60, onchain_mvrv=365, onchain_asopr=365, onchain_puell=365,
    onchain_rhodl=365, onchain_addr_ratio=365, fear_greed=270

m2 keeps the shared default (90) -- its own Stage 1 period already matches
the default, so no override entry is needed. Every other search mechanic
(same 17 indicators, same min_weight_floor=0.1/max=1.0, same 2-point coarse
grid + joint local refine, same spawn-based multiprocessing worker) is
unchanged from ``run_aggregate_reweight_full17.py`` -- only ``load_inputs``
differs.

Usage:
    uv run python scripts/run_aggregate_reweight_full17_fixed_index.py
"""
...
OUT_PATH = DIGIQUANT_ROOT / ".scratch" / "aggregate_reweight_full17_fixed_index_result.json"
...
# Real Stage 1 per-indicator periods for the macro/on-chain extras -- m2 is
# omitted (its Stage 1 period already matches the shared 90-day default, so
# no override entry is needed; build_extra_indicators/_window_for falls back
# to the shared `window` kwarg for any name absent from this dict).
...
def main() -> None:
    ...
    print(f"extra_windows override (real Stage 1 periods): {EXTRA_WINDOWS}")
    ...
    out = {
        ...
        "extra_windows_override": EXTRA_WINDOWS,
