#!/usr/bin/env python3
"""Full 11-indicator SDCA recalibration, Stage 4: walk-forward OOS validation.

Final gate of the pipeline: Stage 1 (per-indicator period search) -> Stage 2
(floor=0.1/max=1.0 aggregate reweight, weekly_monthly_rsi=1.0 +
weekly_monthly_macd=1.0 dominant) -> Stage 3 (wide-knee curve fit,
risk_adjusted_return 42.83 in-sample vs 29.34 published baseline) -> this
script scores the single frozen (weights + oscillators + curve) candidate
on the same 3-fold walk-forward split used for the documented
"+84.90% OOS curve_simulator / +84.78% OOS nautilus" baseline.

Diagnostic only. Does not touch RESEARCH_STATE.md or settings.json -- per
the standing gate, report the full table to Chris for explicit accept
before any production write, even if beats_flat_dca_oos=True here.

Usage:
    uv run python scripts/run_full_recalibration_stage4.py
"""
...
# Stage 1 winning construction periods, same run.
...
# Wide-knee curve winner from run_full_recalibration_stage3.py.
FROZEN_SHAPE_PARAMS = {
    "buy_max_rate": 31.2168,
    "buy_knee_risk": 26.7586,
    "sell_knee_risk": 79.552,
    "sell_max_rate": 8.0908,
    "buy_curvature": 2.5491,
    "sell_curvature": 4.8858,
}
...
def run(cache_dir: Path = DEFAULT_CACHE_DIR) -> None:
    ...
    result_cs = run_sdca_walk_forward(
        ...
        evaluator_label="curve_simulator/full_recalibration_stage2_3",
        extra_z=extra_z,
    )
    ...
    result_nt = run_sdca_walk_forward(
        ...
        evaluator_label="nautilus/full_recalibration_stage2_3",
        extra_z=extra_z,
    )
    ...
    print(
        f"{'full_recalibration (curve_sim)':>40} | OOS {result_cs.mean_oos_vs_flat_dca_pct:6.2f} "
        f"| holdout {cs_holdout:6.2f}"
    )
    print(
        f"{'full_recalibration (nautilus)':>40} | OOS {result_nt.mean_oos_vs_flat_dca_pct:6.2f} "
