...
against the Stage 2 winning index for the 4-Bitview-only on-chain mix (no
``onchain_addr_ratio``).

Isolation trial: the 8-dim mix (validated baseline + 4 Bitview on-chain
series + CoinMetrics ``onchain_addr_ratio``) was rejected at Stage 4
(``run_onchain_addr_ratio_walk_forward.py``, ``beats_flat_dca_oos=False``,
mean OOS ~-20%, driven by an infeasible fold). This freezes the 7-dim mix
without ``addr_ratio`` (``run_onchain_expanded_reweight.py``'s Stage 2
winner: power_law=0.325, m2=0.1, dxy=0.1, onchain_mvrv=1.0,
onchain_asopr=0.1, onchain_puell=0.1, onchain_rhodl=1.0) to check whether
the fold-1 blowup was specific to ``addr_ratio`` or a structural property of
stacking multiple on-chain indicators at weight=1.0.

Per the playbook, curve fitting must never reuse an old shape against a
changed index ("index-then-curve") -- this is a materially different index
from the 8-dim one, so a fresh curve fit is required here.

This does NOT reuse ``curve_optimize.load_frozen_index`` / directly:
that only accepts one shared ``window`` across every onchain ratio
indicator, but Stage 1 found each series wants a different window
(1095/1095/1095/730 days). Manually reconstructs the index via
``stage_a.risk_from_weighted_z``, same pattern as
``run_onchain_addr_ratio_curve_search.py``.

Diagnostic only -- does not touch settings.json or RESEARCH_STATE.md.
Report the full table to Chris for explicit accept first.

Usage:
    python scripts/run_onchain_4bitview_curve_search.py
"""
...
from digiquant.strategies.sdca.optimize import load_sdca_extra_z
from digiquant.strategies.sdca.optimize import load_sdca_extra_sources
...
def build_frozen_index(cache_dir: Path) -> tuple[pl.Series, pl.Series, pl.Series]:
    ...
    # m2/dxy don't need per-indicator window overrides in this mix (both
    # floor-pinned at 0.1); reuse the standard loader for those, then
    # overwrite the 4 onchain ratio series with their Stage-1-winning windows.
...
def run(cache_dir: Path = DEFAULT_CACHE_DIR) -> None:
    ...
    print("=== Stage 3: wide-knee curve search on Stage 2 winning index (4-Bitview-only) ===\n")
