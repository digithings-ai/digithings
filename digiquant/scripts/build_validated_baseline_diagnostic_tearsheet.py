#!/usr/bin/env python3
"""Diagnostic tearsheet for RESEARCH_STATE.md's "Current best validated candidate".

Weights `power_law=1.0, m2=0.5, dxy=0.5`, published `btc_optimized` curve
(buy knee 24.1, sell knee 71.9), dated 2026-09-03, officially validated at
+84.90% OOS (`curve_simulator`) / +84.78% OOS (`nautilus`). This script never
writes `settings.json` or RESEARCH_STATE.md's "Current best validated
candidate" section, and never pushes to Supabase.

Purpose: Chris asked to compare "the current version of the strategy" against
its full calibration history in one place -- this is that reference point,
rendered in the same `TearsheetData` schema the live site uses, via a
local-only `next dev` build. Output goes to a distinct diagnostic slug
(`btc_sdca_validated_baseline`), never `btc_sdca`.

Params source: `.scratch/baseline_relative_rescore.json`'s `validated_baseline`
candidate `best_params` -- a frozen snapshot from this session's
baseline-relative re-score (`run_baseline_relative_rescore.py`, Phase 3,
post band-crossing-fix -- see RESEARCH_STATE.md item 10), reconstructed via
the codebase's own `shape_from_params` / `composite_weights_from_params`
(the same functions `score_trial_on_folds` uses internally) rather than
reading `settings.json` presets live, so this tearsheet can't silently drift
if settings.json changes after this script was written.

IMPORTANT: that same re-score, run under the band-crossing-fixed rails, does
NOT reproduce the +84.90% banner number -- RESEARCH_STATE.md item 10 Phase 3
already documents this as +21.73% duration-weighted OOS, `beats_baseline_oos
=False`. See the notes baked into the output JSON and the printed summary
below. This script surfaces that already-ledgered discrepancy visually; it
does not resolve it or touch the banner.

Usage:
    uv run python scripts/build_validated_baseline_diagnostic_tearsheet.py
"""
...
def main() -> None:
    ...
    entry = rescore["validated_baseline"]
    ...
    mean_oos_rescore = candidate["mean_oos_vs_flat_dca_pct_duration_weighted"]
    sensitivity = candidate["sensitivity"]
    delta_pct = entry["delta_mean_oos_vs_flat_dca_pct"]
    beats_baseline_oos = entry["beats_baseline_oos"]
    baseline_mean_oos = entry["baseline"]["mean_oos_vs_flat_dca_pct_duration_weighted"]
    print(f"Validated-baseline shape: {shape}")
    print(f"Validated-baseline weights: {weights.model_dump()}")
    print(
        f"Fresh rescore mean_oos_vs_flat_dca_pct={mean_oos_rescore:+.2f}%  "
        f"beats_flat_dca_oos={candidate['beats_flat_dca_oos']}  "
        f"sensitivity_stable={sensitivity['stable']} "
    ...
    overlays["curve_knees"] = SdcaCurveKnees(
        ...
        preset="btc_optimized (validated baseline, 2026-09-03)",
    ...
    notes = [
        "DIAGNOSTIC ONLY -- rendered so this can be inspected visually and "
        "compared against every other SDCA calibration attempt. This IS "
        "RESEARCH_STATE.md's official \"Current best validated candidate\" "
        "(power_law=1.0, m2=0.5, dxy=0.5; published btc_optimized curve; "
        "buy knee 24.1, sell knee 71.9; dated 2026-09-03). It is not "
        "published to digiquant.io and settings.json was never modified.",
        "RESEARCH_STATE.md's official record: +84.90% OOS (curve_simulator), "
        "+84.78% OOS (nautilus) -- both evaluators agree once run fresh "
        "under the code as of 2026-09-03. That verdict stands as the "
        "canonical answer until Chris explicitly accepts a change.",
        "IMPORTANT CAVEAT: this session's later baseline-relative re-score "
        "(run_baseline_relative_rescore.py Phase 3, post band-crossing-fix -- "
        "already documented in RESEARCH_STATE.md item 10), run with the SAME "
        f"weights and curve, does NOT reproduce that number -- it shows "
        f"mean_oos_vs_flat_dca_pct={mean_oos_rescore:+.2f}%, still "
        f"beats_flat_dca_oos={candidate['beats_flat_dca_oos']} but now LOSES "
        f"to the naive risk50-linear reference curve ({baseline_mean_oos:+.2f}"
        f"%, delta={delta_pct:+.2f}%, beats_baseline_oos={beats_baseline_oos}) "
        f"and fails the sensitivity-neighbor stability gate "
        f"(max_abs_delta_oos_pct={sensitivity['max_abs_delta_oos_pct']:.2f} "
        f"vs a {sensitivity['spike_threshold_pct']:.1f} threshold). This is "
        "already-recorded information (not new this pass), surfaced here "
        "visually rather than a downgrade of the canonical record -- see "
        "RESEARCH_STATE.md item 10 Phase 3 and this pass's own ledger entry.",
    ...
    dca_kwargs = {
        ...
        "kind": "dca",
        # RESEARCH_STATE.md's canonical, Chris-accepted verdict for this
        # candidate is True -- the freshest rescore's instability is
        # flagged in `notes` above, not silently used to override it here.
        "beats_flat_dca_oos": True,
    }
