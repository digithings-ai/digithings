#!/usr/bin/env python3
"""Diagnostic tearsheet for Blend Approach 1 (Round 2's index + a fresh round-8-style curve search).
...
literal reading: freeze Task #93 Round 2's 17-indicator weights exactly as
scored, then re-run round 8's own full-resolution feasibility-aware curve
search on top of that fixed index
(``scripts/run_blend_approach1_round2index_round8curve.py``), then validate
with ``run_sdca_walk_forward_vs_baseline``.

Result: the new curve search does NOT recover round 8's own performance --
candidate mean_oos_vs_flat_dca_pct=-13.84% (both weightings),
beats_flat_dca_oos=False, sensitivity.stable=False. It still beats the
risk50-linear baseline (delta_mean_oos_vs_flat_dca_pct=+35.79%,
beats_baseline_oos=True) but loses outright to flat DCA -- REJECT under the
standing accept gate, worse than both round 8 (+52.01% OOS, numerically
beats flat DCA, fails sensitivity) and Task #93 Round 2 (numerically beats
flat DCA, fails sensitivity). This is a genuine, surprising result: Round
2's index shape does not carry round 8's edge through a fresh curve search.
...
``.scratch/blend/approach1_round2index_round8curve.json``'s
``candidate.best_params`` (the output of
``run_blend_approach1_round2index_round8curve.py``), reconstructed via
``shape_from_params``/``composite_weights_from_params`` so this tearsheet is
provably the exact config that was scored.

Usage:
    uv run python scripts/build_blend1_diagnostic_tearsheet.py
"""
...
RESULT_PATH = DIGIQUANT_ROOT / ".scratch" / "blend" / "approach1_round2index_round8curve.json"
...
# Duplicated verbatim from run_blend_approach1_round2index_round8curve.py --
# the authoritative oscillator/window config for Task #93 Round 2's index.
...
def main() -> None:
    ...
    delta_pct = result["delta_mean_oos_vs_flat_dca_pct"]
    beats_baseline_oos = result["beats_baseline_oos"]
    print(f"Blend1 shape: {shape}")
    print(f"Blend1 weights: {weights.model_dump()}")
    print(
    ...
    notes = [
        ...
        "Approach 1: Task #93 Round 2's 17-indicator weights frozen exactly "
        "as scored, then a fresh full-resolution feasibility-aware curve "
        "search (round 8's own search method) on top of that fixed index. "
        "It is not published to digiquant.io and settings.json was never "
        "modified.",
        ...
        "Surprising result: re-searching the curve on top of Round 2's frozen "
        "index does NOT recover round 8's edge -- this candidate loses "
        "outright to flat DCA (-13.84% OOS), worse than both round 8 "
        "(+52.01% OOS, numerically beats flat DCA but fails sensitivity) and "
        "Task #93 Round 2 itself (numerically beats flat DCA but fails "
        "sensitivity). Round 2's valuation-index shape does not carry round "
        "8's performance through a fresh curve search.",
        ...
        "17-indicator composite weights frozen from Task #93 Round 2 (see "
        "indicator_weights below); dominant members onchain_addr_ratio/"
        "onchain_rhodl (1.0), onchain_mvrv (0.95), weekly_monthly_macd (1.0).",
    ...
    dca_kwargs = {
        ...
        "label": "BTC-SDCA (Blend 1: Round 2 index + Round 8 curve)",
        "kind": "dca",
        # False per this field's own documented semantics ("do not claim an
        # OOS win") -- Blend 1 lost outright to flat DCA (numerically
        # negative), on top of failing the sensitivity-stability gate.
