#!/usr/bin/env python3
"""Stage 3 against a genuinely fold-causal risk index (not the leaky full-history one).

Context (see project_digithings_sdca_strategy.md): every candidate tried on
this project -- 12+ dead ends across weight/indicator/window/objective axes,
including a fold-aware/drawdown-capped Stage-3 *objective*
(``run_fold_drawdown_capped_curve_search.py``) -- converges to the same
~50-53% fold-1 (COVID crash) walk-forward OOS drawdown. Root cause found by
reading ``walk_forward.py``'s ``_score_fold()``: real Stage 4 refits BTC
power-law rails on each fold's IS-only window before scoring that fold's OOS
("OOS never enters rails_fitter -- a full-history fit would leak and
mislead"). ``curve_optimize.py``'s ``load_frozen_index()`` -- used by
*every* Stage-3 curve search in this project, including the objective
experiment above -- builds its index from ``BtcPowerLawRiskModel(load_coefficients())``,
a single rails fit on the FULL 2018-2026 history. Stage 3 has therefore
never searched against the same signal Stage 4 uses to accept/reject.

This script builds a piecewise fold-causal index instead: for each of the 3
standing walk-forward folds, fit power-law rails on that fold's own IS
window only (``btc_power_law_rails_fitter``, the same production rails
fitter ``run_sdca_walk_forward`` uses), then stitch those fold-specific
rails together date-by-date so every day's risk value comes from a model
that saw no data past that fold's own IS cutoff -- exactly mirroring what
``_score_fold()`` evaluates, fold by fold. The held-out tail uses a model
fit on the full searchable span (IS+OOS of all 3 folds), since the holdout
is never used for search either way.

Runs the same wide-knee curve search as every prior Stage-3 script against
this stitched index, then validates the winner with a REAL Stage 4
walk-forward (the whole point: does removing the leak make fold 1 finally
feasible?).

Diagnostic only. Does not touch settings.json or RESEARCH_STATE.md.

Usage:
    uv run python scripts/run_fold_causal_curve_search.py
"""
...
class StitchedFoldCausalRiskModel:
    ...
    Segment boundaries come from the SAME ``make_walk_forward_folds`` call
    real Stage 4 uses. Each of the 3 fold segments (IS_0..OOS_0, OOS_1,
    OOS_2) is filled by a model fit on that fold's own IS window
    (``btc_power_law_rails_fitter``, no future data). The holdout tail is
    filled by a model fit on the full searchable span (IS_2 union OOS_2),
    which is fine because the holdout is never used to pick a trial.
    """
...
def run(cache_dir: Path = DEFAULT_CACHE_DIR) -> None:
    ...
    print(f"fold-causal index span: {dates[0]}..{dates[-1]} ({len(dates)} days), weights={resolved.model_dump()}")

    trials = sample_wide_knee_curve_trials(n_random=3000)
    print(f"\nscoring {len(trials)} wide-knee trials against the fold-causal index...")
    scored = []
    ...
    print("\n=== top 5 by risk_adjusted_return (on fold-causal index) ===")
    for s in by_risk_adjusted[:5]:
        print(
            f"  ret={s['total_return_pct']:8.2f}%  full_dd={s['full_dd']:6.2f}%  "
            f"fold1_dd={s['fold1_dd']:6.2f}%  score={s['risk_adjusted_return']:.4f}  "
            f"shape={s['shape'].model_dump()}"
        )

    winner = by_risk_adjusted[0]["shape"]
    winner_params = winner.model_dump()
    print(f"\nWinner shape: {winner_params}")

    print("\n=== Stage 4: real walk-forward of the fold-causal-index winner ===\n")
    ...
    for label, evaluator in (
        ...
        result = run_sdca_walk_forward(
            ...
            evaluator=evaluator,
            evaluator_label=f"{label}/fold_causal_curve",
            extra_z=extra_z,
        )
