#!/usr/bin/env python3
"""Stage 1 of the Phase B indicator-addition playbook: solo-validate onchain_mvrv.

Per ``RESEARCH_STATE.md``'s Phase B procedure (first applied here to the
on-chain metrics added in PR #3855), before a new indicator is allowed to
affect the aggregate composite it must clear a solo-validation gate: soloed
(weight=1, every other indicator at 0) and scored against the same combined
long+medium cycle-overlap objective the rest of the composite is tuned
against (``stage_a.combined_cycle_overlap_score`` via
``weight_search.search_oscillator_periods_by_cycle_overlap``), it must beat
a constant-zero noise baseline. This mirrors
``run_dual_timeframe_composite_search.py``'s Stage 2 for a single new
indicator instead of the full 9/11-indicator set.

``onchain_mvrv``'s only tunable parameter is its rolling-z window (see
``indicator_catalog.onchain_mvrv_z``) -- unlike the price oscillators, there's
no separate weekly/daily period pair to search.
...
requires re-running Stage 2 (floor-diversified reweight) with the candidate
added and Chris's explicit accept.

Usage:
    python scripts/run_onchain_mvrv_solo_search.py
"""
...
# MVRV tracks multi-year halving cycles, not weekly/monthly price swings, so
# the grid skews longer than the price-oscillator searches (60d-4y).
WINDOW_CANDIDATES = [
...
def run(data_path: Path = DEFAULT_DATA_PATH) -> None:
    ...
    if sources.onchain_mvrv_dates is None:
        raise SystemExit(
            "no onchain_mvrv source found -- run fetch_bitview_series(['mvrv'], "
            "cache_dir='data/onchain/bitview') first"
        )
    mvrv_dates, mvrv_values = sources.onchain_mvrv_dates, sources.onchain_mvrv_values
    print(
        f"onchain_mvrv source: {mvrv_dates[0]}..{mvrv_dates[-1]} "
        f"({len(mvrv_dates)} points)\n"
    )
    ...
    def compute_onchain_mvrv_z(p: dict[str, int]) -> list[float | None]:
        return onchain_mvrv_z(
            date_s, mvrv_dates, mvrv_values, window=p["window"],
        ).to_list()

    print("=== Stage 1: onchain_mvrv solo period search (combined objective) ===\n")
    result = search_oscillator_periods_by_cycle_overlap(
        dates,
        indicator_name="onchain_mvrv",
        param_candidates=WINDOW_CANDIDATES,
        compute_indicator_z=compute_onchain_mvrv_z,
