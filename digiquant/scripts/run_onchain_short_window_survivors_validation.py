#!/usr/bin/env python3
"""Short-trailing-window variant of ``run_onchain_survivors_validation.py``.

``run_onchain_risk_floor_diagnostic.py`` found that ``onchain_mvrv``/
``onchain_rhodl`` at their Stage-1-selected windows (1095d/730d) keep fold
1's (2019-07-01..2021-11-20, the COVID-crash OOS window) composite risk
permanently above the frozen curve's ``buy_knee_risk`` -- the strategy buys
0% of capital for the entire 874-day window because the long trailing
windows still "remember" the 2017 bubble and 2019 rally.
``run_onchain_short_window_dead_zone_check.py`` swept shorter windows on
just the risk-index arithmetic (no curve/walk-forward) and found the dead
zone actually breaks at short windows: at 120d, fold-1 COVID-low risk
(29.57) dips just below buy_knee_risk (29.71), vs. 0% time-below-knee at
every window >=365d.

This re-runs the real Stage 2b/3/4 pipeline (survivors-only reweight, fresh
curve fit, 3-fold walk-forward) with ``onchain_mvrv``/``onchain_rhodl`` both
built at a 120d trailing window instead of the original 1095d/730d, to test
whether that dead-zone-breaking behavior actually translates into a passing
OOS candidate, not just a marginally-less-stuck risk index.
...
built at the shortened 120d/120d windows this time, not the original
Stage-1-selected 1095d/730d) and run the existing wide-knee curve search
...
ONCHAIN_WINDOWS = {"onchain_mvrv": 120, "onchain_rhodl": 120}
...
def run(data_path: Path = DEFAULT_DATA_PATH) -> None:
    ...
    result_cs = run_sdca_walk_forward(
        ...
        evaluator_label="curve_simulator/onchain_mvrv_rhodl_short_window",
        extra_z=extra_z,
    )
    ...
    result_nt = run_sdca_walk_forward(
        ...
        evaluator_label="nautilus/onchain_mvrv_rhodl_short_window",
        extra_z=extra_z,
    )
