#!/usr/bin/env python3
"""Diagnostic tearsheet for Task #93 Round 3 (feasibility-aware curve search, REJECT).

Task #93 (RESEARCH_STATE.md item 9, 2026-09-23) ran a full 17-indicator
recalibration pass across three rounds; all three were rejected. Round 3
(commit `54c4a95f9`) kept Round 2's weights and re-ran the curve search with
a feasibility-aware objective (penalizing curves that ever drive capital
deployed negative). Result: `beats_flat_dca_oos=False` -- an unambiguous
REJECT, matching RESEARCH_STATE.md's verdict exactly. This script never
...
`ROUND3_CURVE` / `ROUND23_OSCILLATORS` / `ROUND23_EXTRA_WINDOWS` constants
(this repo's self-contained-script convention -- no cross-script imports),
which are themselves duplicated from the original Round 3 driver script per
...
`.scratch/baseline_relative_rescore.json`'s `task93_round3.candidate`, so
this tearsheet is provably the exact config that was scored.

Usage:
    uv run python scripts/build_task93_round3_diagnostic_tearsheet.py
"""
...
def main() -> None:
    ...
    candidate = rescore["task93_round3"]["candidate"]
    ...
    delta_pct = rescore["task93_round3"]["delta_mean_oos_vs_flat_dca_pct"]
    beats_baseline_oos = rescore["task93_round3"]["beats_baseline_oos"]
    print(f"Task93 Round3 shape: {shape}")
    print(f"Task93 Round3 weights: {weights.model_dump()}")
    print(
    ...
    overlays["curve_knees"] = SdcaCurveKnees(
        ...
        preset="task93_round3_diagnostic",
    ...
    notes = [
        ...
        "Task #93's (RESEARCH_STATE.md item 9, 2026-09-23) Round 3: Round 2's "
        "same 17-indicator weights, re-run with a feasibility-aware curve "
        "search (commit 54c4a95f9) that penalizes curves ever driving "
        "capital deployed negative. It is not published to digiquant.io and "
        "settings.json was never modified.",
        f"Walk-forward vs. flat DCA: mean_oos_vs_flat_dca_pct={mean_oos:+.2f}%, "
        f"beats_flat_dca_oos={candidate['beats_flat_dca_oos']} -- an "
        "unambiguous REJECT, matching RESEARCH_STATE.md's verdict exactly. "
        "Walk-forward vs. the risk50-linear baseline "
        f"(run_baseline_relative_rescore.py): delta_mean_oos_vs_flat_dca_pct="
        f"{delta_pct:+.2f}%, beats_baseline_oos={beats_baseline_oos} (beats "
        "the simple linear baseline but still loses to flat DCA outright). "
        f"Sensitivity-neighbor stability gate also failed "
        f"(max_abs_delta_oos_pct={sensitivity['max_abs_delta_oos_pct']:.2f} "
        ...
        "onchain_mvrv (0.95), weekly_monthly_macd (1.0) -- identical to "
        "Round 2's weights, only the buy/sell curve shape differs.",
    ...
    dca_kwargs = {
        ...
        "label": "BTC-SDCA (Task #93 Round 3 diagnostic — REJECT)",
        "kind": "dca",
