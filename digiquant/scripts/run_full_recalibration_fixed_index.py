#!/usr/bin/env python3
"""Re-run of ``run_full_recalibration.py``'s Stage 4 walk-forward OOS gate on
the CORRECTED final trial (Stage 2b weights + Stage 3b curve shape), built
against the fixed index.

Round 1 (``run_full_recalibration.py``) built its Stage 4 ``extra_z`` via
``load_sdca_extra_z(..., extra_windows=None)`` -- every macro/on-chain extra
(m2, dxy, the four on-chain series, fear_greed) got the shared
``DEFAULT_ROLLING_WINDOW`` (90) regardless of its own Stage 1 period-search
result. Round 1's gate came back ``beats_flat_dca_oos=True`` but
``sensitivity_stable=False`` (max_abs_delta_oos_pct=5.71 vs a 2.0 threshold)
with 2 of 3 OOS folds flagged infeasible under the capital-deployed/drawdown
rails -- REJECTED, per ``run_full_recalibration.py``'s own gate note.

This script is the same Stage 4 walk-forward call
(``optimize.run_sdca_walk_forward`` / ``evaluator=curve_sim.
evaluate_sdca_trial_curve_sim`` / ``rails_fitter=optimize.
btc_power_law_rails_fitter``, 3 expanding-IS/rolling-OOS folds + held-out
tail + +/-5% sensitivity sweep -- identical mechanism, so directly
comparable to Round 1 and to the currently validated baseline's numbers),
but:

  1. threads the newly-plumbed ``extra_windows`` override through
     ``load_sdca_extra_z`` -> ``extra_z_vectors`` -> ``build_extra_indicators``
     (see indicator_catalog.py's ``_window_for``) with each indicator's real
     Stage 1 period -- the SAME override already used and validated by
     ``run_aggregate_reweight_full17_fixed_index.py`` /
     ``run_stage3_curve_search_fixed_index.py`` for Stage 2b/3b themselves:

         dxy=60, onchain_mvrv=365, onchain_asopr=365, onchain_puell=365,
         onchain_rhodl=365, onchain_addr_ratio=365, fear_greed=270

     (m2 omitted -- its Stage 1 period already matches the shared 90-day
     default, so no override entry changes its behavior.)

  2. replaces ``STAGE2_WEIGHTS``/``STAGE3_CURVE`` with the corrected final
     trial: Stage 2b's 17-indicator floor-diversified reweight (fit AGAINST
     this same fixed index) + Stage 3b's wide-knee curve (fit against
     Stage 2b's index in turn) -- both frozen verbatim from their own search
     result files, not re-derived here.

Stage 1 oscillator periods are UNCHANGED from ``run_full_recalibration.py``
(the fixed-index correction only affects the five extras whose window isn't
already controlled by ``SdcaOscillatorSpec`` -- m2/dxy/onchain_*/fear_greed
per that module's documented caveat).
...
Usage:
    uv run python scripts/run_full_recalibration_fixed_index.py
"""
...
OUT_PATH = DIGIQUANT_ROOT / ".scratch" / "full_recalibration_fixed_index_walk_forward_result.json"

# ---------------------------------------------------------------------------
# Stage 1 winners -- unchanged from run_full_recalibration.py. The fixed-index
# correction only touches the five extras whose rolling-z window isn't
# controllable via SdcaOscillatorSpec (build_extra_indicators() only takes
# one shared window=90 for those; see EXTRA_WINDOWS below).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Fixed-index correction: real Stage 1 per-indicator periods for the
# macro/on-chain extras, in place of Round 1's silent window=90 default for
# all of them. m2 omitted (Stage 1 period already matches 90, no-op).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 2b winner (run_aggregate_reweight_full17_fixed_index.py, fit against
# the fixed index above) -- the corrected final trial's weights.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 3b winner (run_stage3_curve_search_fixed_index.py, wide-knee curve
# search frozen against Stage 1 oscillators + Stage 2b weights above, on the
# fixed index). bounds_used=WIDE_KNEE_SEARCH_BOUNDS (sell_max_rate<=95,
# unwidened) -- a widened-ceiling run (sell_max_rate<=100) landed on an
# almost identical shape (sell_max_rate=17.6098, risk_adjusted_return=46.3905)
# per that search's own result file; not used here.
# ---------------------------------------------------------------------------
...
def print_stage2_summary() -> None:
    print("\n=== Stage 2b: 17-indicator floor-diversified aggregate reweight, fixed index (frozen) ===")
...
def print_stage3_summary() -> None:
    print("\n=== Stage 3b: wide-knee curve search, fixed index, frozen against Stage 1+2b (frozen) ===")
    for field, value in STAGE3B_CURVE.model_dump().items():
        print(f"  {field}: {value:g}")
    print("  bounds_used: WIDE_KNEE_SEARCH_BOUNDS (sell_max_rate<=95, unwidened)")


def build_stage4_trial() -> dict[str, float | int | str]:
    """Stage 2b weights + Stage 3b curve shape as one ``run_sdca_walk_forward`` trial dict."""
