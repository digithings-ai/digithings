#!/usr/bin/env python3
"""Full 11-indicator SDCA recalibration, Stage 2: aggregate weight reweight.

Reweights the 11 indicators (with their Stage 1 optimized periods) using
floor=0.1 and max=1.0 diversification bounds. Runs a coarse 3-point grid
(0.1, 0.55, 1.0) then local refinement around the winner.

Usage:
    uv run python -m scripts.run_full_recalibration_stage2
"""
...
# Stage 1 winners (from run_full_recalibration_stage1.py output)
STAGE1_WINNERS = {
...
# Coarse grid only (refine pass skipped, using multi-ratio sensitivity instead)
...
def run(data_path: Path = DEFAULT_DATA_PATH) -> None:
    ...
    # Compute all Stage 1 winner z-series
    print("=== Stage 2: Aggregate reweight (floor=0.1, max=1.0) ===\n")
    ...
    weekly_rsi_z = rsi_confluence_z(
        date_s, price_s, **STAGE1_WINNERS["weekly_rsi"]
    )

    weekly_macd_z = macd_confluence_z(
        date_s, price_s, **STAGE1_WINNERS["weekly_macd"]
    )

    monthly_rsi_z = monthly_rsi_confluence_z(
        date_s, price_s, **STAGE1_WINNERS["monthly_rsi"]
    )

    monthly_macd_z = monthly_macd_confluence_z(
        date_s, price_s, **STAGE1_WINNERS["monthly_macd"]
    )

    sma_band_z = sma_band_confluence_z(
        date_s, price_s, **STAGE1_WINNERS["sma_band"]
    )
    ...
    # Build extra_z dict (excluding power_law, m2, dxy which are handled separately)
    ...
    # Sensitivity check at different long:medium ratios (using coarse winner)
    ...
    # Extract results for all ratios
    ...
    print(f"\n=== Stage 2 Complete ===")
    print(f"Stage 2 winner weights ready for Stage 3 (curve optimization).")
