...
portfolio turns research into an allocation memo and rebalance decision. It replaces the single-call `phase7c_analyst` + `phase7d_pm` pair with a seven-phase flow: thesis review, vehicle mapping, opportunity screening, blinded per-ticker analysis, cyclic analyst↔PM deliberation, and the PM memo.
...
- A pipeline-level collector appends each `RecessRequest` to `state.phase_portfolio.recess_requests` via the list-append reducer.
- After the fan-in barrier on `phase_h6`, a conditional router checks `len(state.phase_portfolio.recess_requests) > 0`:
...
## 3. State additions to `ResearchState`

A new nested block is added to [`state.py`](../../../src/digiquant/research/state.py):
...
class ResearchState(BaseModel):
    ...
    phase_portfolio: PhasePortfolioState = Field(default_factory=PhasePortfolioState)
```
...
- **Schema:** [`market-thesis-exploration.schema.json`](../../portfolio/templates/schemas/market-thesis-exploration.schema.json).
...
- **Schema:** [`thesis-vehicle-map.schema.json`](../../portfolio/templates/schemas/thesis-vehicle-map.schema.json).
...
- **Validation:** every `thesis_id` in the output must appear in `state.phase_portfolio.thesis_review.reviewed_theses` with non-`CLOSED` status, or in `market_thesis_exploration.theses`.
...
- **Schema:** [`asset-recommendation.schema.json`](../../portfolio/templates/schemas/asset-recommendation.schema.json).
...
- **Schema (transcript):** [`deliberation-transcript.schema.json`](../../portfolio/templates/schemas/deliberation-transcript.schema.json).
- **Schema (session index):** [`deliberation-session-index.schema.json`](../../portfolio/templates/schemas/deliberation-session-index.schema.json).
- **Fields:**
  - Transcript `body`: `trigger_summary: list[str]`, `rounds: list[DeliberationRound]`, `final_decisions: list[FinalDecision]`.
  - `DeliberationRound`: `label`, `sections: list[{heading, markdown}]` — canonical heading values, in round order: `"analyst"` (analyst-present), `"pm_challenge"` (PM push-back), `"analyst_defense"` (analyst rebuttal), `"pm_decision"` (PM commit to converge/continue), optional `"recess_reason"` (populated only when `recess_triggered=True`). Writers MUST use these exact strings so downstream readers can index by heading without a regex. Plus portfolio-added `converged: bool`, `recess_triggered: bool`, `deep_dive_document_key: str | None`, `round_number: int`.
...
- **Schema:** [`pm-allocation-memo.schema.json`](../../portfolio/templates/schemas/pm-allocation-memo.schema.json).
...
Each H-phase's Supabase adapter (to be added in W2-A) writes to both `documents` (full JSON payload, backward-compat) **and** the first-class tables from migration 024 (subset of structured fields for indexed query). research's `supabase_io.py` gains one writer function per H-phase.
...
### 5.1 Migration 025 — portfolio doc_type additions (stub, implemented in W2-A)
...
```sql
-- digiquant/supabase/migrations/025_portfolio_doc_types.sql
...
W2-A applies 025 to dev before W2-B / W2-D can green their persistence tests. No other portfolio writer is blocked on 025.

---

## 6. Delta-run behavior

Triage ([`triage.py`](../../../src/digiquant/research/triage.py)) is extended with portfolio-tier rules in W2-H:
...
- `portfolio_thesis_drift` (for h2/h3) — evaluates whether h1 produced any status transitions.
- `portfolio_ticker_filter` (for h5/h6) — evaluates per-ticker instead of per-segment; the gate returns a `Carried` marker keyed by ticker. Requires extending the gate signature from `(state, segment) → Carried | None` to also accept `(state, ticker) → Carried | None`; W2-H tracks the split.
- `portfolio_memo_gate` (for h7) — regenerate iff `state.phase_portfolio.deliberation_sessions` has any fresh (non-Carried) entry for this run.
...
- Callers move to `state.phase_portfolio.asset_recommendations`.
...
- New input to `phase7d`: `state.phase_portfolio.pm_allocation_memo`.
...
- **Batching plan (Wave 3):** h5 per-ticker and h6 round batches become single Anthropic Batches submissions; `deep_dive_batch` likewise. The `NodeSpec(batch=True)` flag lands in W3-C — portfolio only needs to declare the intent in node metadata, which the pipeline builder picks up.
- **Per-run cost telemetry:** portfolio phases emit token counts into `state.phase9_evolution.cost_by_phase[phase_name]`. Phase 9 post-mortem prints a cost trend line per phase so regressions show up against prior baselines.
