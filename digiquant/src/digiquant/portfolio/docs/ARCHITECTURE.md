...
> deliberation summaries, PM direction memo, sized book, and terminal booking via commit.
>
...
preflight + preflight_reflect (research)
  ...
  → portfolio thesis–commit (thesis-first)
  → publish_phase (research artifacts only)
```

portfolio terminal persist is **commit `commit_run`** (in-graph): `positions`, `nav_history`,
`theses` / `thesis_vehicles` sync, portfolio brief (weights from sizing), `decision_log`
...
## thesis–commit path map

| Step | Node | Module | Edit behavior | Output |
|------|------|--------|---------------|--------|
| **thesis** | `portfolio/thesis/market-review` | `phases/thesis.py` | `edit` active market theses | `theses` rows + `documents.document_key=thesis/thesis-review`. Consumes `digest_briefing_for_portfolio` (`date` / `body` / `regime_label`), not DigestSnapshot JSON findings. |
| **market** | `portfolio/thesis/market-exploration` | `phases/market.py` | `edit` exploration doc | market thesis proposals |
| **vehicle_map** | `portfolio/thesis/vehicle-map` | `phases/vehicle_map.py` | `full`/`edit` | `thesis_vehicles` |
| **screener** | `portfolio/thesis/opportunity-screener` | `phases/screener.py` | deterministic | focus roster (held + mapped + unlinked), capped by a **regime-adaptive budget**; publishes `documents.document_key=opportunity-screener` (`doc_type` `opportunity_screen`) |
| **analyst** | `portfolio/asset-analyst` (×N) | `phases/analyst.py` | `skip`/`edit`/`full` per ticker | unified `AnalystPayload` + WP11.2 `ticker_evidence_bundles` (base build before provider; cite on new forecasts; optional `PortfolioGraphDeps.evidence_bundle_store` append when injected; `DIGIQUANT_EVIDENCE_BUNDLE_WRITER=off` kill switch) |
| **deliberation** | `portfolio/deliberation` (×N) | `phases/deliberation.py` | cyclic PM↔analyst sub-graph; WP11.3 `DeliberationSelection` (`DIGIQUANT_H6_SELECTION_MODE`); WP11.4 bounded missing-fact amendment via shared `evidence_bundle_store` | `deliberation_transcript` + summary (+ amendment/carry provenance) |
| **direction** | `portfolio/pm-direction` | `phases/direction.py` | `edit` prior memo | `PMDirectionMemo` — **no weights**; optional `confidence` ∈ [0, 1] |
| **sizing** | `portfolio/risk-sizing` | `phases/phase7e_risk_sizing.py` | no LLM | `phase_portfolio.sized_book` (sole weight owner; calibrated μ/σ × PM confidence) |
| **commit** | `portfolio/commit-run` | `phases/commit.py` | no LLM | positions, nav, brief, `decision_log` |
...
mutating weights. sizing attaches the report to `phase_portfolio.pre_trade_risk_report`
...
identity equals the final book fingerprint (same extractor as commit:
`commit_io.weights_from_sized_book`). Fail-soft omission does not change
the sized book. commit (`commit_run`) validates report identity under
...
without blocking (covered by unit tests; #2824). commit never recomputes the report.
...
book weights, `PreTradeRiskReport`, and minimal commit metadata with a SHA-256
...
reruns or mutates sizing/commit). The module must not import challenger optimizer, replay,
...
forbidden imports (Supabase, commit I/O, network clients, live Nautilus,
...
infeasible seed. Shadow-only — never imported by `chain.py`, sizing, or commit; no
...
Never wired into production sizing/commit; no auto-promotion or config write.

**Phase 2 lock surface (#2820 / Integration 2.1):**
`tests/dq/portfolio/test_phase2_allocation_contracts.py` and
`phase2_e2e_fixtures.py` lock Gate 2 composition for WP8–WP10 (calibrated sizing,
...
blinded role contexts, deliberation selection round floor, telemetry reconciliation)
...
analyst/deliberation lineage round-trip with graph-level planner-node guards.

### thesis review persistence

thesis (`persist_thesis_review`) refreshes status/evidence on **existing market**
...
topics before persistence. New topics start `ACTIVE`; updates preserve thesis's same-run status,
or the prior nonterminal status when thesis did not review that topic. A `PAUSED` topic remains
...
thesis it expresses. It is resolved at **creation time by analyst**
(`upsert_vehicle_thesis_from_analyst` → `resolve_primary_market_thesis`) from the
reliable `thesis_vehicles` map (vehicle_map's ticker → market-thesis mapping): primary =
lowest `candidate_rank`, falling back to the most recent prior mapping for a
carried held name. This replaced a same-date vehicle_map back-fill that structurally
never fired (the `vehicle-{ticker}` row does not exist when vehicle_map runs), which left
every vehicle thesis null-linked in prod. The link is **self-healing** — analyst
...
## screener dispatch budget (regime-adaptive, Stage 2 — #1043 / #1017)
...
The cost-safe invariant above held for `budget`, but not for the roster: from the day vehicle_map
...
**Known limitation.** There is **no conviction signal anywhere in the vehicle_map output** —
...
(rebalance prior-weight vs sizing target when that payload is at hand, else prior
direction, else `long`/`flat`), rank, and confidence as a percent. sizing maps memo +
feasibility constraints → sized weights. On the calibrated path rank is **not** a
size input (μ/σ/reliability + 12% vol budget + 5% grid). sizing then scales each long
by direction `confidence` (cash-first; missing rows on a mixed roster → 0.5, never 1.0;
pre-WP-G memos that omit confidence on every long skip the haircut). The UI
must never imply #1 = largest weight.

---

## deliberation sub-graph
...
- `deliberation_analyst_response` — analyst answers in meeting prose (not a second analyst report)
...
(`skills/deliberation/analyst-response-full.md`) — **not** analyst `asset-analyst`.
...
quiet (#925): `skip` — carry prior deliberation summary into direction; fresh
...
decision features, provider/round budget) from structured features after analyst:
...
| Mode | Behavior |
|---|---|
| `shadow` (default) | Record selection; run **full incumbent** deliberation (fingerprint skip still applies) |
...
| `off` | No actuation; incumbent deliberation with `incumbent_fallback` provenance |

Planner failure falls back to full incumbent deliberation (typed `incumbent_fallback`), never
an unrecorded skip. `weight_pct` / materiality features are selection-only and must
not enter provider prompts. Does not replace screener roster/exploration ownership.
WP11.4+ (durable lineage round trip) still open — WP11 incomplete.

### Research attention after screener — WP13.4 (#2930)

After screener materializes `focus_roster`, `portfolio/research_attention.py` invokes
`plan_research_attention` over ticker targets only (helper at screener end — not a graph
...
floor; screener output is byte-identical across modes.

| Phase | Enforced behavior |
|---|---|
| analyst | `carry` → skip provider; `metric_patch` → deterministic structured patch; `deep_refresh` → force full; `challenge`/`section_patch` → incumbent edit path |
| deliberation | Re-route after analyst features: `challenge` runs deliberation; other modes carry with `attention_carry` |
...
per-role allowlists. analyst delta-evidence policy, deliberation bundle/amendment-only evidence,
and direction attention-decision sections are enforced at compile time — not yet wired
into provider calls (WP14.2–14.4). Prose is never authoritative over structured
state.

### Bounded missing-fact amendment — WP11.4 (#2908)

deliberation no longer runs generic ``live_search`` web grounding. When the PM names exactly
...
the immutable analyst base — never broad re-grounding.
...
- **Document.** `payloads.deliberation_summaries` publishes **no** `bear_thesis`; mirroring
  ...
  Successful deliberation chats publish the turns under both `transcript` (canonical) and `rounds`
...
- **Sizing.** sizing caps the name's conviction at `SizingCaps.min_conviction`
  (`phase7e._cap_unchallenged_convictions`) — applied to **both** the memo and the legacy
  branch, since direction writes a memo on every production run. Capping *at* the bar, not below
...
Every portfolio LLM call site (thesis–vehicle_map via `thesis_common`, analyst via `portfolio_common`, deliberation
deliberation turns, direction memo, 7D debate/PM, phase 9 evolution) is wrapped: a
...
phase-appropriate fallback — direction carries the prior memo re-dated (held names it misses
are covered by the #1649 carry), deliberation carries the analyst stance, analyst/thesis skip the
item, 7D empties the debate arm, legacy PM skips (sizing prefers the direction memo anyway).
...
## commit-run: coherence, held-carry, and observability (#932 / #1030 / #1555 / #1649)

commit is the sole terminal writer. Before it books, `commit_io.coherence_errors` runs two
fail-closed checks over the sizing `sized_book` weights:

1. every prior holding is either in the book with positive weight **or** explicitly `flat`
   in the direction memo (no silent drop of an owned name);
2. every open position has an analyst doc **or** is `flat` **or** is a deliberately
   carried held name (`commit_io.carried_held_tickers`).

When advisory position risk fields are enabled, commit resolves `positions.horizon_days` from
...
sizing's carry injection (`phase7e_risk_sizing._held_carry_weights`) and commit's exemption, so
the two can never diverge — covers:

- **screener-gated** (#1030/#1555): the staleness gate moves a quiet held name into
  `focus_roster_excluded` and dispatches no analyst, so it never reaches the direction memo.
- **Memo-unaddressed** (#1649): the direction memo's roster omits a held name entirely (neither
...
sizing carries both classes into the sized book at their current drifted weight *before* the
...
invariant on the finished dict regardless of cause — the 2026-07-22 22:54 run reached commit
...
stays out and commit still fails closed.
...
**Commit is observable.** A book sizing materializes but commit does not persist (coherence
...
commit `PhaseError` can't trigger on its own. Both flags are emitted structurally in the
...
| Writer | When | Owns |
|---|---|---|
| commit `commit_io.book_portfolio` | commit time, ~12:00–14:00 UTC | the **provisional** row: NAV as of the latest close available *before* `run_date`, plus `cash_pct` / `invested_pct`, which commit alone owns |
...
by having commit write the later value — at commit time that close does not exist yet.

What commit *must* get right is its anchor. Because the cron restates row `D` to "NAV as of
D's close", `_prior_nav` already embeds the move up to the prior book date. So the return
commit applies is measured **over the interval from the prior book date to the last close
...
- 22 `run_diagnostics` rows cover that window and **18 of them report `status='ok'`**.
  All 18 carry the commit coherence check (1) failure in `error_summary`:
  `portfolio_commit/portfolio/commit-run: held ticker <T> missing from book and not
  flat in direction` (EWT on most dates; EWT, IJR, UUP and TLT by 07-16). Grep production for **that
...
portfolio thesis–direction use `build_grounding` / `build_thesis_grounding` with phase-scoped tool
...
- [`PORTFOLIO_SUBGRAPH.md`](PORTFOLIO_SUBGRAPH.md) — historical Wave 2 spec (topology now shipped as thesis–commit)
...
3. **portfolio reasoning failures are counted, not just logged.** deliberation degrades one ticker per
