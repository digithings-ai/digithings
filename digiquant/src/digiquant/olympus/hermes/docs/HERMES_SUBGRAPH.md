...
A new nested block is added to [`state.py`](../../../src/digiquant/olympus/atlas/state.py):
...
Triage ([`triage.py`](../../../src/digiquant/olympus/atlas/triage.py)) is extended with Hermes-tier rules in W2-H:
...
**Decision: replace.** The existing [`phase7c_analyst.py`](../../../src/digiquant/olympus/atlas/phases/phase7c_analyst.py) emits a minimal `AnalystPayload` (`conviction_score`, `stance`, `thesis`, `risks`). `phase_h5` emits the richer `AssetRecommendation` governed by the existing [`asset-recommendation.schema.json`](../../hermes/templates/schemas/asset-recommendation.schema.json) — bull/bear cases, context block, verdict with `recommended_weight_pct`, thesis linkage.
...
[`phase7d_pm.py`](../../../src/digiquant/olympus/atlas/phases/phase7d_pm.py) currently reads `state.phase7c_analysts` directly and re-derives allocations. Under Hermes, phase_h7 writes the allocation memo and phase7d becomes a **pure transform**:
