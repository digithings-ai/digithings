...
edit-mode continuity) → portfolio thesis–commit (thesis-first) → `commit_run`. Cost is controlled by
...
- **Operator full refresh:** `--refresh-scope all` (manual `workflow_dispatch` / CLI)
