...
| [`scripts/execute_at_open.py`](../../scripts/execute_at_open.py) | `position_events` (OPEN/EXIT/TRIM/ADD/**HOLD**) from `rebalance_table` + **`positions`** (extra tickers on book get HOLD so Activity matches the book) + `price_history.open`. Every positions diff compares against the **prior committed book** — `max(positions.date) < --date` (#1743) — not the prior calendar weekday: the pipeline skips days, and an empty comparison book made EXIT unreachable and turned held names into spurious OPENs. `_prior_trading_date` remains the **document-date** walk (Fri `rebalance_decision` → Mon open) and must not be repointed. |
