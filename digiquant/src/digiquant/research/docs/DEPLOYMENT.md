...
| `DIGIQUANT_STALE_FULL_DAYS` | `7` | Prior gap > N calendar days → `full` rewrite instead of `edit` (retired read-alias `OLYMPUS_STALE_FULL_DAYS`) |
| `DIGIQUANT_BELIEFS_BACKLOG` | `20` | Additional trigger for a **full** beliefs rewrite when unfolded `decision_log` rows exceed the threshold. House runs already publish a **daily short fold**. (retired read-alias `OLYMPUS_BELIEFS_BACKLOG`) |
