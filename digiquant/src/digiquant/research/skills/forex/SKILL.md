...
> DB-first: read DXY and FX levels from the injected `market_context` and published `daily_snapshots.market_data` / `documents.payload`, and anchor the dollar view with `get_macro_series` (`DTWEXBGS`, `DGS10`, `DGS2`). The per-pair watchlist series (`FX/EUR`, `FX/GBP`, `FX/JPY`, `FX/CAD`; directions `USD_per_EUR`, `USD_per_GBP`, `JPY_per_USD`, `CAD_per_USD`) surface in the snapshot payload — market history is not readable through `query_data` (#3780).
