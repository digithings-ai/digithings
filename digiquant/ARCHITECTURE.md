...
| `sdca/btc_power_law.py` | `BtcPowerLawRiskModel` — the first concrete `RiskModel` (#1082): fits 7 quantile rails (`q01`…`q99`) as `price_q(t) = 10 ** (c + a*x + b*x**2)`, `x = ln(days_since_genesis(t)) - mu`, one quantile regression (`statsmodels.QuantReg`, lazily imported) per rail. `rails()`/`rails_full()` sort each row's fitted quantiles ascending (rearrangement method) so independently-fit curves never cross. `fit_btc_power_law()`/`save_coefficients()`/`load_coefficients()` handle fitting and JSON persistence; `load_coefficients()` prefers the real fit (`btc_power_law_coefficients.json`, **committed #3173**) and falls back to the checked-in synthetic placeholder (`btc_power_law_coefficients.example.json`) with a warning. The `digiquant_fit_btc_power_law` MCP tool is the orchestration layer — this module has no data-fetching or MCP dependency of its own. `low_quantile`/`high_quantile` (default `q10`/`q95`) pick which fitted rails map to the protocol's `low`/`high`. **#3173 validation (Coinbase daily BTC-USD, 2015-07-20–2026-08-29, 4059 bars):** in-sample coverage of close vs rearranged rails is q01 1.03 / q10 10.05 / q25 25.03 / q50 50.01 / q75 74.97 / q95 94.95 / q99 98.97 (target = the quantile, within 0.1pp); zero rearrangement crossings on the fit window and on a 5-year forward date grid; 2026-08-29 close 78234 sits at z=+1.45 / risk 25.9 (value band), between q10=64563 and q50=93538 — not a 1st/99th-percentile sanity failure. The q10/q95 default is kept: coverage matches, and the 10% cheap vs 5% rich clip matches a book that should max-buy more often than max-sell. **Stability finding (not a reason to change the methodology here):** refitting only through 2020-12-31 or 2023-12-31 moves the 2024-01-01…2026-08-29 risk *band* on 87.8% of those days (mean \|Δrisk\| ≈ 41). A 2023-truncated model puts 2026-08-29 at risk 100 (high rail ~43k) vs the full-sample value-band. Truncated quadratic log-time fits do not extrapolate; #3174 must refit per walk-forward fold or bound this rails-leakage. The owner's railway reference artifact returned 404 on 2026-08-30 and is not ground truth. |
