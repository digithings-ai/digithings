...
| `digiquant_build_sdca_risk_index` | Builds the SDCA `date`/`risk` parquet from a `RiskModel` + cached daily prices (`history_cache.py`, never a bespoke fetch) and writes it for `SdcaStrategy.risk_path` (#3168, #3175). `risk_model` selector: `btc_power_law` / `generic_valuation` / `rolling_z` (see `sdca/providers.py`). Returns `{path, row_count, date_start, date_end, null_risk_days}` or `{"error": ...}` |
...
| `sdca/quantile_rails.py` | Shared QuantReg rail fitting, rearrangement, `QuantileCoefficients`, history/calendar guards, and log-space rail widening (#3175). `numpy`/`statsmodels` stay lazily imported. `MIN_FIT_HISTORY_DAYS = 730`; `REFERENCE_SPAN_DAYS = 2922` (8 years) is the generic-trend full-confidence span. |
| `sdca/btc_power_law.py` | `BtcPowerLawRiskModel` — BTC-specific configuration of `quantile_rails` (#1082, #3175): genesis-anchored log-time, quadratic basis. Behaviour and public error strings are unchanged from #1082. `load_coefficients()` prefers the real fit (`btc_power_law_coefficients.json`) and falls back to the synthetic placeholder with a warning. |
| `sdca/generic_valuation.py` | `GenericValuationRiskModel` (#3175) — log10(price) vs calendar days since the asset's first cached bar. Form `log_linear` or `log_quadratic` is recorded on the coefficients. When fit span < `REFERENCE_SPAN_DAYS`, `widen_factor = REFERENCE_SPAN_DAYS / fit_span` scales log-space spreads from the median so a short fit is less confident, not more. Refuses below `MIN_FIT_HISTORY_DAYS`. |
| `sdca/rolling_z.py` | `RollingZRiskModel` (#3175) — causal rolling log-price mean ± z·std as median/low/high. Fallback for series below the trend-fit floor. First `min_samples-1` days are null rails (no-trade). |
| `sdca/providers.py` | `resolve_sdca_risk_model(name, ...)` (#3175) — string selector used by `digiquant_build_sdca_risk_index`. Ladder: `btc_power_law` → `generic_valuation` → `rolling_z`. Unknown names raise `ValueError("unknown risk_model '...'")`. To add a provider: implement `rails()`, append the name, branch, and cover `build_risk_index` via the selector. RS-driven risk (#1084) stays out. |
...
**RiskModel provider ladder (#3175).** Pick the most specific model the
asset can support: (1) a *bespoke* power-law when the asset has a
genesis-anchored thesis (`btc_power_law`), (2) a *generic* log-price
trend when history clears `MIN_FIT_HISTORY_DAYS` (`generic_valuation`,
log-linear or log-quadratic, rails widen below ~8 years of span), (3) a
*rolling-z* fallback when even a trend fit is unjustifiable. The MCP
tool reaches all three through `resolve_sdca_risk_model`. Adding a
fourth provider is a new `rails()` implementation + a selector branch —
not a copy of `btc_power_law.py`. RS-driven risk stays with #1084.
