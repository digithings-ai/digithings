...
4. **Edit-mode extension pattern** (`digiquant.olympus.edit_mode`):
   ...
   - Track C glass-box (#1945): `digiquant.olympus.attention_plan_io` publishes
     `attention-plan` documents for Pipeline (refresh reasons + profile pin). Graph
     call-site wiring is a follow-up — do not fabricate UI rows.5. **Hermes extension pattern** (H1–H9): add phases via `build_hermes_phases_thesis`; wire
