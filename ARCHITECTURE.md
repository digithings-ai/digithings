...
DigiSearch exposes document search via HTTP + MCP (digisearch_query tool) for DigiGraph/DigiFlow agents
...
DigiGraph,Cognitive orchestration & memory,DigiClaw ↔ DigiQuant ↔ DigiSearch
DigiQuant,High-perf research → backtest → live,DigiGraph only
DigiSearch,RAG, document search, ingestion,DigiGraph, DigiFlow (HTTP + MCP)
