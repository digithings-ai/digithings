...
DIGIDEV_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$DIGIDEV_DIR/digidev.yml"
...
_has_file ".gitlab-ci.yml" && DETECTED_GITLAB=1
...
subs = {k: v for k, v in os.environ.items() if k.startswith('DIGIDEV_TMPL_')}

for key, value in subs.items():
    placeholder = '{{' + key[len('DIGIDEV_TMPL_'):] + '}}'
...
'
  ...
  # Write config safely via Python to avoid YAML injection from user-supplied values.
  python3 - "$CONFIG_FILE" \
    "$WIZ_PROJECT" "$WIZ_ORG" "$WIZ_REPO" "$WIZ_DEFAULT" "$WIZ_MAIN" \
    "$WIZ_HANDLES" "$WIZ_STACK" "$WIZ_GIT_PLATFORM" "$WIZ_ISSUE_TRACKER" \
    "$WIZ_COMM" "$WIZ_DB" "$WIZ_AGENTS_LIST" \
    "$WIZ_COMPONENTS_YAML" "$ALLOWED_HOSTS_LIST" <<'PY'
import sys
from pathlib import Path

out, project, org, repo, default_b, main_b, handles, stack, git_plat, tracker, \
  comm, db, agents_list, components_yaml, hosts_yaml = sys.argv[1:]
...
org_name: {q(org)}
repo_name: {q(repo)}
default_branch: {q(default_b)}
main_branch: {q(main_b)}
contributor_handles: {q(handles)}
tech_stack: {q(stack)}

# Integrations — set during setup wizard (re-run with --setup-mcp to change)
git_platform: {q(git_plat)}
issue_tracker: {q(tracker)}
communication: {q(comm)}
database: {q(db)}
agents: {q(agents_list)}
...
# Regex matching paths that require a Human-Approved-By git trailer to push.
# Example: "(live_trading|execute_trade|place_order)"
...
# Parse YAML with Python and export as DIGIDEV_TMPL_* env vars for substitution.
...
def sh_export(key, val):
    ...
    print(f"export DIGIDEV_TMPL_{key}='{safe}'")
...
# Each component as a separate YAML list option for GitHub issue template dropdowns
...
sh_export('COMPONENTS_DROPDOWN', components_dropdown)
...
if [ "$DIGIDEV_TMPL_PROJECT_NAME" = "YOUR_PROJECT" ]; then
  die "digidev.yml: project_name is not set. Edit digidev/digidev.yml first."
fi
if [ "$DIGIDEV_TMPL_ORG_NAME" = "YOUR_ORG" ]; then
  die "digidev.yml: org_name is not set. Edit digidev/digidev.yml first."
fi
if [ "$DIGIDEV_TMPL_REPO_NAME" = "YOUR_REPO" ]; then
  die "digidev.yml: repo_name is not set. Edit digidev/digidev.yml first."
fi

echo ""
echo "digidev installer"
echo "  project : $DIGIDEV_TMPL_PROJECT_NAME"
echo "  repo    : $DIGIDEV_TMPL_ORG_NAME/$DIGIDEV_TMPL_REPO_NAME"
echo "  branch  : $DIGIDEV_TMPL_DEFAULT_BRANCH ← task/* ← module/*"
echo "  stack   : $DIGIDEV_TMPL_TECH_STACK"
echo ""

# ── Claude Code surface ───────────────────────────────────────────────────────

echo "→ Claude Code settings"
install_file "$DIGIDEV_DIR/templates/claude-settings.json" ".claude/settings.json"

# ── Claude hooks ──────────────────────────────────────────────────────────────

echo "→ Claude Code hook scripts"
hooks_src="$DIGIDEV_DIR/templates/hooks/claude"
...
install_file "$DIGIDEV_DIR/templates/hooks/git/pre-push.sh" "scripts/hooks/pre-push.sh"
make_exec "scripts/hooks/pre-push.sh"

# ── GitHub templates ──────────────────────────────────────────────────────────

echo "→ GitHub issue + PR templates"
install_file "$DIGIDEV_DIR/templates/github/ISSUE_TEMPLATE/agent_task.yml" \
             ".github/ISSUE_TEMPLATE/agent_task.yml"
install_file "$DIGIDEV_DIR/templates/github/ISSUE_TEMPLATE/config.yml" \
             ".github/ISSUE_TEMPLATE/config.yml"
install_file "$DIGIDEV_DIR/templates/github/PULL_REQUEST_TEMPLATE.md" \
             ".github/PULL_REQUEST_TEMPLATE.md"

echo "→ GitHub workflows"
install_file "$DIGIDEV_DIR/templates/github/workflows/claude-code-dispatch.yml" \
             ".github/workflows/claude-code-dispatch.yml"
install_file "$DIGIDEV_DIR/templates/github/workflows/auto-assign-copilot.yml" \
             ".github/workflows/auto-assign-copilot.yml"
install_file "$DIGIDEV_DIR/templates/github/workflows/route-issues-to-projects.yml" \
             ".github/workflows/route-issues-to-projects.yml"

# ── Project manifest ──────────────────────────────────────────────────────────

echo "→ agents.yml project manifest"
install_file "$DIGIDEV_DIR/templates/agents.yml" "agents.yml"

echo "→ AGENTS.md"
install_file "$DIGIDEV_DIR/templates/AGENTS.md" "AGENTS.md"

# ── Makefile.include ──────────────────────────────────────────────────────────

echo "→ Makefile.include"
install_file "$DIGIDEV_DIR/Makefile.include" "Makefile.digidev" \
...
for doc in README.md SECURITY.md QUALITY.md OPTIMIZATION.md ACCURACY.md; do
  install_file "$DIGIDEV_DIR/docs/scoring/$doc" "docs/scoring/$doc"
done

# ── Per-component AGENTS.md ───────────────────────────────────────────────────

echo "→ Component AGENTS.md files"
python3 - "$CONFIG_FILE" "$DIGIDEV_DIR/templates/COMPONENT_AGENTS.md" \
...
for comp in components:
    ...
    content = content.replace('{{DEFAULT_BRANCH}}', os.environ.get('DIGIDEV_TMPL_DEFAULT_BRANCH', 'develop'))
...
skills_src="$DIGIDEV_DIR/templates/claude/skills"
...
cmds_src="$DIGIDEV_DIR/templates/claude/commands"
...
scripts_src="$DIGIDEV_DIR/scripts"
scripts_dst="scripts"
...
if issue_tracker == 'jira':
    # mcp-atlassian is a Python package; use uvx for zero-install execution
    servers['jira'] = {
        'command': 'uvx',
        'args': ['mcp-atlassian'],
        'env': {
    ...
    # Linear uses a hosted SSE endpoint, not a local server
    servers['linear'] = {
        'type': 'sse',
...
if issue_tracker == 'jira':
    steps.append(("Jira MCP (Python — install with uv)", [
    ...
    steps.append(("Linear MCP (hosted SSE — no install needed)", [
        "# Linear uses OAuth, not API keys.",
        "claude mcp add --transport sse linear https://mcp.linear.app/sse",
        "# This opens a browser to authenticate with your Linear account.",
