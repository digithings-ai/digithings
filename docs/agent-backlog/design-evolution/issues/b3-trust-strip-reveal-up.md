...
Add shared **`TrustStrip`** (logo/proof row) and **`reveal-up`** enter-animation utilities — Cursor hero trust line + Graphite friction reducers + Graphite glide reveals ([`copy-patterns.md`](../../../../packages/design/references/scans/copy-patterns.md)).
...
- [ ] Integrate with existing `packages/design/scroll-trigger.js` or `site/reveal.js` — document trigger contract
