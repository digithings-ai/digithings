# Vendor map — cursor cloud agent (Kairos staging secrets)

> Transparent map of where each secret lives and how to re-fetch.
> **No secret values in this file.** Label: **cursor cloud agent**.
> Local files: `/workspace/.local/secrets/cursor-cloud-agent-*.env` (gitignored).
...
| Item | Value |
|------|-------|
| Primary inbox (existing) | `digithings@agentmail.to` |
| Vendor onboarding inbox (2026-08-30) | `cursor-cloud-agent6060@agentmail.to` |
...
| Local file (when ready) | `.local/secrets/cursor-cloud-agent-stripe.env` |
...
| Sandbox recipients | Authorize `cursor-cloud-agent6060@agentmail.to` (+ `digithings@agentmail.to`) |
| Test send | Sending → Send email, or `python -m digiquant.notify.dispatch --require-mailgun` |
| Local file (when ready) | `.local/secrets/cursor-cloud-agent-mailgun.env` |
...
| Signup attempt email | `cursor-cloud-agent6060@agentmail.to` |
...
| Local file (when ready) | `.local/secrets/cursor-cloud-agent-alpaca.env` |
...
| Signup attempt | email `cursor-cloud-agent6060@agentmail.to`; form filled (US, Reddit) |
...
| Local file (when ready) | `.local/secrets/cursor-cloud-agent-google.env` |
...
## After secrets land (agent resume)

```bash
export SUPABASE_ACCESS_TOKEN="$(tr -d '\n' < .local/secrets/cursor-cloud-agent-supabase-pat)"
# source vendor env files then:
...
## Browser tabs left open for human

1. Stripe register + hCaptcha
2. Mailgun signup (filled) + reCAPTCHA error
3. Alpaca signup (filled) + Turnstile
