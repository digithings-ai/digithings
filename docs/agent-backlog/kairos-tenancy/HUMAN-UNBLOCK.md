...
# when all three .local/secrets/digithings-{stripe,notify,alpaca}.env exist:
...
# this agent. Exit 3 until a counting 17 9/10/11/12 * * * schedule. Never workflow_dispatch.
...
PATH="$PWD/.venv/bin:$PATH" python -m digiquant.notify.dispatch --require-notify
...
GitHub secrets include `CORE_SUPABASE_*` plus notify names. House daily stays
...
| `CLOUDFLARE_EMAIL_API_TOKEN` | Dedicated Cloudflare API token with **Email Sending: Edit** (not the broad deploy token); env EMPTY |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare account id (existing repo secret) |
| `NOTIFY_FROM` | Verified From address, bare or `Name <addr@domain>` |
...
mig **107** + settings `ensureCallerWorkspace` — Agentmail JWT settings **200**. Personal workspace exists. Ops may elevate to `studio` for vault/overlay probes until Stripe prices land (document clearly — **not** Stripe-sourced). GitHub user’s personal WS remains **`free`** (2026-08-30 Settings E2E; ops elevate not applied). Notification prefs point at Agentmail inbox for digest when notify lands.
...
Do **not** resume Stripe / Alpaca signup until human replies captcha done (or says continue). Screenshots and map: [`VENDOR_MAP.md`](VENDOR_MAP.md).
