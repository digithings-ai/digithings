...
| Agent unlock (2026-08-30) | **Partial** — `sbp_` + EF vault/APP_URL + settings **v18** + **GitHub Auth Enabled** + **Agentmail Auth** (`auth.users=1`); docs through [#3214](https://github.com/digithings-ai/digithings/pull/3214) merged | Waiting `PARTIAL_UNLOCK`. JWT settings → `WORKSPACE_FORBIDDEN` (no memberships). Vendors + Cursor env PAT paste still blocked. #3183 left draft. |
