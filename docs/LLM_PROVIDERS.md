...
- **Cheaper Inference** — optional hosted OpenAI-compat discount gateway (`api.cheaperinference.com/v1`). Opt-in house savings via `DIGI_HOUSE_UPSTREAM=cheaperinference` + LiteLLM overlay; see [providers/cheaperinference.md](providers/cheaperinference.md). OpenRouter remains fallback for sonar / `:online` / missing slugs. Distinct from OmniRoute.
