...
| *(spec, not installed)* `docs/agent-backlog/kairos-tenancy/kairos-cron-check.workflow.yml` | Pipeline: execution cron check | schedule (daily 12:15 UTC), dispatch | Overlay + execution sync + notify `--check`/`--dry-run` probe; **must not** share the house portfolio job. Copy to `.github/workflows/execution-cron-check.yml` on a `chore/`/`feat/` branch (`cursor/*` cannot write workflows) | Spec pinned by `tests/scripts/test_digiquant_cron_workflow.py` | none |
| `test-research-graph.yml` | Test: research graph | workflow_call | Unit tests + lint for research + portfolio trees; installs the full workspace from `uv.lock` (`uv sync --frozen`) | Working | `digiquant/src/digiquant/{research,portfolio}/**`, `tests/dq/{research,portfolio}/**`, `pipeline-digiquant.yml` |
...
| `ci.yml` | CI | push (main/develop), PR | Orchestrator: per-component tests + score + e2e-contract + nautilus-smoke + research-graph + pip-audit + ruff/scripts/baseline/provider_review + compose-validate + `actionlint` (repo-wide workflow lint) + `frontend-canon` (unconditional canon guard, #1434) | Working | none |
...
| `test-dashboard.yml` | Test: dashboard | workflow_call | Dashboard lint + vitest + build | Working | `cloudflare/dashboard/**`, design |
| `test-web.yml` | Test: web apps | workflow_call | Lint `digithings-web` + `digiquant-web` + `design-reference` (root workspaces); typecheck `design-reference` (`tsc --noEmit`); vitest for `digithings-web`, `@digithings/web`, `@digithings/digichat-ui`; frontend canon guard (`check_frontend_canon.py`) | Working | `cloudflare/{digithings-web,digiquant-web,digichat-ui,digiweb/web,digiweb/design,digiweb/reference}/**`, `package.json`/lock |
...
| `test-digichat.yml` | Test: digichat | workflow_call | digichat (Next.js) lint + tests | Working | `cloudflare/digichat/**`, `cloudflare/digiweb/design/**`, `package.json` |
| `release-please-digichat.yml` | Release please: digichat | push (`develop`) | Track digichat version + changelog from Conventional Commits on `develop` (decoupled from image publish, #1343). Retargeted off `module/digichat` in #1948 — that branch had not moved since `digichat-v0.6.0`, so two releases were hand-cut and left untagged. Root `package-lock.json` workspace version is bumped via `extra-files` (#1974); the consistency tests ride `ruff_and_scripts` on release PRs. | Working | `cloudflare/digichat/**`, release-please config/manifest |
| `publish-digichat-image.yml` | Publish: digichat image | push (`main`), dispatch | Build + push digichat image to GHCR once a version reaches `main`; skips if that version tag already exists (#1343) | Working | `cloudflare/digichat/**` |
...
| `smoke-langsmith.yml` | Smoke: LangSmith | dispatch only | Readiness check (#687): `LANGSMITH_API_KEY` auth + `@traceable` nesting before enabling tracing on research workflows | Working | none |
| `pipeline-research-metrics.yml` | Pipeline: research metrics refresh | schedule (daily, post-EOD), dispatch | Deterministic Polars/SQL recompute of `portfolio_metrics` + `position_attribution` the dashboard reads; zero LLM cost; runs after EOD price ingest | Working | none |
| ~~`pipeline-digiquant-backfill.yml`~~ | *(retired)* | — | Deleted in #4053: migration 127 dropped the Supabase `price_history`/`price_technicals` tables it backfilled; the R2 refresh/backfill owns ingest now | — | none |
| `db-migrate.yml` | db-migrate | push (`main`), dispatch | Apply pending dashboard Supabase migrations to prod; forward-only, the `digithings_schema_migrations` ledger is the SOLE skip gate (a copy-forward bootstrap migrates the old `olympus_schema_migrations` rows before the gate) (the `BASELINE_THROUGH` baseline branch was deleted in #1814 — it silently recorded a new low-numbered file as applied without running its DDL); one transaction per file on the unwrapped path; `production` env (human gate) (#1016) | Working | `digiquant/supabase/migrations/**` |
...
### 0. GitHub Agentic Workflows (gh-aw) — retired

**Status: retired (2026-08-05).** Copilot coding-agent automation and its gh-aw
sources (`.github/workflows/*.md` → `*.lock.yml`) were deleted in #1904.
ADR-0017 is superseded history — do not reintroduce `gh aw compile` without a
new ADR.

**Replacement CI drift check** (still in `ci-docs.yml`):

```bash
python scripts/agents_init.py --check   # or: make agents-init (then --check)
```

That job also runs markdown link integrity + `make vault-check`. There is no
`gh aw compile --trial` step anymore; the workflow header documents the removal
(#1182).

**Agent `test_cmd` alignment:** `agents.yml` `components[].test_cmd` values use
the same path selectors as `.github/workflows/test-<component>.yml` (for
example `pytest tests/ds/ -m unit …`, not `-k digisearch`). Enforced by
`tests/agents/test_agents_yml_test_cmds.py`.

**Local verify:**

```bash
python scripts/agents_init.py --check
pytest tests/agents/test_agents_yml_test_cmds.py -m unit -v --tb=short
```
...
This pattern is used by the dashboard pipeline workflow (`pipeline-digiquant.yml`) and is now required for all new issue/comment creation steps.
...
For production pipelines where mid-run cancellation would corrupt state (research baseline, monthly synthesis), set `cancel-in-progress: false` and use a named group without `${{ github.ref }}`.
...
`*.lock.yml` is ignored in `.github/actionlint.yaml` as a belt-and-suspenders
holdover after gh-aw retirement — no lock files ship in-tree today (#1182).
