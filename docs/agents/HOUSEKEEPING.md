...
| npm dependency CVEs | `security-npm-audit.yml` (dispatched weekly by the cron Worker) + `ci.yml` — `npm-audit` lane | weekly Mon 06:37 UTC + on PR | Runs `npm audit` on the root `apps/*` + `packages/*` workspace closure, blocks HIGH/CRITICAL, sites lower severities (accepted advisories in `npm-audit-ignore.txt`) |
