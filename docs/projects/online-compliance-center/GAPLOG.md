...
- Static `digithings_docs` + `occ_help` seeds ship in `cloudflare/digithings-stack-cloudflare/container/seed/`; oneshot `seed_chroma` ingests both into Chroma **before** digisearch starts (vault `seed-*.md` refreshed on every boot)
