...
- **Components:** `apps/digichat`, `digigraph`, `digillm`, `config/`
...
- `ARCHITECTURE.md` (`apps/digichat`) — update the BYOK section's provider table and note the new catalog file as the source of truth for the allowlist.
...
- `apps/digithings-web/lib/providerSettings.ts` and `components/ProviderSettings.tsx` — not converged onto the new catalog in this PR.
...
- **`agents.require_tool_calls` set but the visitor's BYOK model can't tool-call** — now that `tool_choice="required"` is wired end-to-end (digigraph forwards it to `run_tools()`, which forces it onto every wire call in the streaming path production actually takes), this is a **hard error**, not graceful degradation: a provider that rejects forced tool use for a model returns an error for the whole request instead of the model quietly answering without tools. The UI badge is advisory, not enforced — matches this repo's existing stance that `is_tool_use_capable_model()` gates a *pool selection policy*, not a hard runtime block — but once `require_tool_calls` is on, a badge miss now surfaces as a failed request rather than a silently tool-less answer.
...
- Converging `apps/digithings-web`'s independent BYOK picker (`providerSettings.ts`) onto the new catalog.
