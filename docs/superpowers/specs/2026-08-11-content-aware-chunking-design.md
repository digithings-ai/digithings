...
- `Document` gains an optional field:
  `segments: list["Segment"] = field(default_factory=list)`. An **empty list**
  (the default) means "no known structure" — identical to today's behavior
  everywhere downstream. (An earlier draft of this spec said
  `list[Segment] | None = None`. The implementation uses `field(default_factory=list)`
  instead, mirroring the adjacent `chunks` field and `core/models.py`
  conventions; empty and absent need not be distinguished because every
  consumer guards with `if not doc.segments:`. Owner-confirmed 2026-08-11.)
  Note these are **stdlib dataclasses, not pydantic** — see the plan's Global
  Constraints.
...
- **New generic heading segmenter** — a small pure function,
  ...
  content with no headings (callers fall back to the empty-`segments` behavior).
  Applied to:
...
- Everything else (CSV, DOCX, short plaintext, short marketing HTML with no
  headings) leaves `segments` empty — unchanged, one-chunk/one-note behavior.
