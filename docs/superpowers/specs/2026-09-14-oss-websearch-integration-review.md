...
- Request shape: `WebSearchRequest{query, search_type="auto", num_results=8 (1-100), category=None,
  contents_text=False, output_schema=None, system_prompt=None}` (`server.py:770-781`).
- Tool split: `digisearch_web_search` = shallow live search → `POST /v1/web_search` (EXA-gated, 503 without key);
