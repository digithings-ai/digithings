...
- **Components:** `apps/digichat`, `digigraph`
...
- The **digigraph** adapter (`apps/digichat/src/lib/adapters/digithings/stream.ts`)
  — used by digithings.ai/chat and OCC (`digithings.ai/chat/occ`), both on the
  Profile A stack.
- The **Foundry** adapter (`apps/digichat/src/lib/adapters/foundry/stream.ts`)
...
- `ARCHITECTURE.md` updated for `digigraph` (new state field) and
  `apps/digichat` (new tenant flag + dual-backend contract).
