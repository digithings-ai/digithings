...
- **Visitor chat:** `digithings.ai/chat` — Cloudflare Pages (`apps/digithings-web`) native `DigiChatSession` + digivault **Pages Function** (`functions/api/chat.ts`).
...
2. Deploy digithings-web (build mirrors `apps/digithings-web/functions` → repo-root `functions/`).
3. No `NEXT_PUBLIC_DIGICHAT_EMBED_ORIGIN` required for `/chat`.
4. Optional deferred: Workers Paid + `apps/digichat-cloudflare/` if later promoting to Containers.
   *Superseded 2026-08-06 — that scaffold was deleted unused; see ADR-0018 historical note 4.*

### Local

```bash
cp apps/digithings-web/.dev.vars.example apps/digithings-web/.dev.vars
