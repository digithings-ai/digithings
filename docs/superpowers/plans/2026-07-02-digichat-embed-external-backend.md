...
- digithings repo: every PR links a GitHub issue (`task/<N>-slug` branch); PRs go into `module/digichat`, never directly into `develop`; run `make score` on staged changes before the PR; the implementing PR **always requires human review** (new external network dependency, per CLAUDE.md); update `apps/digichat/ARCHITECTURE.md` after interface changes; never hand-edit `.claude/`.
...
3. **Create the task branch:**
   ...
   cd apps/digichat && npm ci   # workspace deps (run from repo root if workspaces hoist: npm ci at root)
   ```
4. **Test command sanity check:** `cd apps/digichat && npx vitest run src/lib/embed-ip-rate-limit.test.ts` — expect PASS (proves the vitest setup works before you write anything).
...
## Group A — digithings (`~/Code/digithings`, all paths relative to `apps/digichat/`)

### Task 1: Embed tenant registry module

**Repo:** digithings
**Files:**
- Create: `apps/digichat/src/lib/embed-tenants.ts`
- Test: `apps/digichat/src/lib/embed-tenants.test.ts`
...
Run: `cd ~/Code/digithings/apps/digichat && npx vitest run src/lib/embed-tenants.test.ts`
Expected: FAIL — cannot resolve `./embed-tenants`.

- [ ] **Step 3: Write the implementation**

```ts
// apps/digichat/src/lib/embed-tenants.ts
/**
...
- Test: `apps/digichat/src/lib/security-headers.test.ts` (extend, keep existing assertions)
...
```ts
// Append to apps/digichat/src/lib/security-headers.test.ts
...
```bash
git add apps/digichat/src/lib/security-headers.ts apps/digichat/src/lib/security-headers.test.ts
...
**Repo:** digithings
**Files:**
- Create: `apps/digichat/src/lib/external-relay-stream.ts`
- Test: `apps/digichat/src/lib/external-relay-stream.test.ts`
...
```ts
// apps/digichat/src/lib/external-relay-stream.test.ts
import { describe, it, expect, afterEach, vi } from "vitest";
...
```ts
// apps/digichat/src/lib/external-relay-stream.ts
/**
...
```bash
git add apps/digichat/src/lib/external-relay-stream.ts apps/digichat/src/lib/external-relay-stream.test.ts
...
- Test: `apps/digichat/src/lib/embed-chat-tenant.test.ts` (create if absent; if a test file already exists, extend it and keep its cases green)
...
```bash
git add apps/digichat/src/lib/embed-chat-tenant.ts apps/digichat/src/lib/embed-chat-tenant.test.ts
...
- Test: `apps/digichat/src/app/api/chat/route.test.ts` (extend — follow the file's existing mocking patterns exactly)
...
```ts
// Add to apps/digichat/src/app/api/chat/route.test.ts
...
**Repo:** digithings
**Files:**
- Create: `apps/digichat/src/app/api/embed/tenant-config/route.ts`
- Test: `apps/digichat/src/app/api/embed/tenant-config/route.test.ts`
...
```bash
git add apps/digichat/src/app/api/embed/tenant-config
...
**Repo:** digithings
**Files:**
- Create: `apps/digichat/src/hooks/use-embed-tenant-config.ts`
...
```ts
// apps/digichat/src/hooks/use-embed-tenant-config.ts
"use client";
...
```bash
git add apps/digichat/src/hooks/use-embed-tenant-config.ts apps/digichat/src/app/embed/page.tsx
...
```bash
git add apps/digichat/src/app/embed/page.tsx
...
**Repo:** digithings
**Files:**
- Modify: `apps/digichat/ARCHITECTURE.md` (required by repo rules after interface changes)
- Modify: `apps/digichat/README.md` (env var table/section, if one exists — otherwise ARCHITECTURE.md's env section only)
...
```bash
git add apps/digichat/ARCHITECTURE.md apps/digichat/README.md
...
```bash
cd ~/Code/digithings/apps/digichat
