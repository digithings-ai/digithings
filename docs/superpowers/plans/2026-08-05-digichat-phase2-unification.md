...
- Run digichat tests from `apps/digichat` with `npx vitest run <path>`. Run digichat-ui tests from `packages/digichat-ui` with `npx vitest run <path>`.
...
| File | Responsibility |
|---|---|
| `packages/digichat-ui/src/types.ts` | Extend `VaultHitSummary`; add `brief` to `DigiChatActivity` |
| `packages/digichat-ui/src/components/ChatActivities.tsx` | Render tier/year/snippet hits + brief block |
| `packages/digichat-ui/src/styles/session.css` | Hit meta + brief styles |
| `packages/digichat-ui/src/ChatActivities.test.ts` | Unit coverage for new kinds |
| `apps/digichat/src/lib/chat-activity.ts` | Allowlist for tier/year/snippet + brief; detail gate; projector |
| `apps/digichat/src/lib/chat-activity.test.ts` | Allowlist / detail / projector tests |
| `apps/digichat/src/lib/digigraph-activity-map.ts` | Typed `rag_sources` / `graph_update` → `ActivitySpan[]` |
| `apps/digichat/src/lib/digigraph-activity-map.test.ts` | Mapper fixtures |
| `apps/digichat/src/lib/stream-digigraph-trace.ts` | Use mapper; delete dual-emit / `emitLegacyTracePart` |
| `apps/digichat/src/lib/stream-digigraph-trace.test.ts` | Assert no `data-digigraphTrace`; rich activity |
| `apps/digichat/src/components/chat-panel.tsx` | Consume `data-digichatActivity` via `ChatActivities`; delete DigigraphTraceBlock / RagSourcesTrace / ResearchBriefTrace |
| `apps/digichat/src/components/digigraph-trace.tsx` | **Delete** (unused legacy card) |
| `apps/digichat/src/lib/embed-tenants.ts` | Parse/validate `digivault` backend variant |
| `apps/digichat/src/lib/digivault-env.ts` | Resolve per-tenant env-name refs fail-closed |
| `apps/digichat/src/lib/digivault-ip-rate-limit.ts` | 60/min/IP wrapper over `checkBffRateLimit` |
| `apps/digichat/src/lib/digivault-vault.ts` | Supabase `search_architecture_notes` + context builder (body server-side only) |
| `apps/digichat/src/lib/digivault-byok.ts` | Free pool + full BYOK route resolution (incl. Gemini) |
| `apps/digichat/src/lib/digivault-stream.ts` | Agentic loop → AI SDK UI stream + activity spans |
| `apps/digichat/src/lib/digivault-ndjson-adapter.ts` | CF NDJSON event → internal server events (parity) |
| `apps/digichat/src/lib/fixtures/digivault/*` | Recorded NDJSON + vault RPC + golden spans/text |
| `apps/digichat/src/app/api/chat/route.ts` | Branch `digivault`; stop passing `emitLegacyTracePart` |
| `apps/digichat/src/hooks/use-byok-key.ts` (+ settings UI) | Add `gemini` to BYOK providers for parity |
| `apps/digichat/ARCHITECTURE.md` | Document digivault backend + dual-emit removal |
...
**Files:**
- Modify: `packages/digichat-ui/src/types.ts`
- Modify: `packages/digichat-ui/src/components/ChatActivities.tsx`
- Modify: `packages/digichat-ui/src/styles/session.css`
- Create: `packages/digichat-ui/src/ChatActivities.test.ts`
...
Run: `cd packages/digichat-ui && npx vitest run src/ChatActivities.test.ts`
...
In `packages/digichat-ui/src/types.ts` replace:
...
In `packages/digichat-ui/src/components/ChatActivities.tsx`, update `tool_result` hit list items and add `case "brief"`:
...
Append to `packages/digichat-ui/src/styles/session.css`:
...
```bash
git add packages/digichat-ui/src/types.ts \
  packages/digichat-ui/src/components/ChatActivities.tsx \
  packages/digichat-ui/src/styles/session.css \
  packages/digichat-ui/src/ChatActivities.test.ts
...
**Files:**
- Modify: `apps/digichat/src/lib/chat-activity.ts`
- Modify: `apps/digichat/src/lib/chat-activity.test.ts`
...
Append to `apps/digichat/src/lib/chat-activity.test.ts`:
...
```bash
git add apps/digichat/src/lib/chat-activity.ts apps/digichat/src/lib/chat-activity.test.ts
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `apps/digichat/src/lib/chat-activity.ts` (`toDigiChatActivity`)
- Modify: `apps/digichat/src/lib/chat-activity.test.ts`
...
**Files:**
- Create: `apps/digichat/src/lib/digigraph-activity-map.ts`
- Create: `apps/digichat/src/lib/digigraph-activity-map.test.ts`
...
Run: `cd apps/digichat && npx vitest run src/lib/digigraph-activity-map.test.ts`
...
```bash
git add apps/digichat/src/lib/digigraph-activity-map.ts \
  apps/digichat/src/lib/digigraph-activity-map.test.ts
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `apps/digichat/src/lib/stream-digigraph-trace.ts`
- Modify: `apps/digichat/src/lib/stream-digigraph-trace.test.ts`
...
Run: `cd apps/digichat && npx vitest run src/lib/stream-digigraph-trace.test.ts`
...
```bash
git add apps/digichat/src/lib/stream-digigraph-trace.ts \
  apps/digichat/src/lib/stream-digigraph-trace.test.ts
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `apps/digichat/src/components/chat-panel.tsx`
- Delete: `apps/digichat/src/components/digigraph-trace.tsx` (unused)
...
4. Delete `apps/digichat/src/components/digigraph-trace.tsx`.

- [ ] **Step 4: Run tests**

Run:
```bash
cd apps/digichat && npx vitest run src/lib/chat-activity.test.ts
```
Expected: PASS

Also run a typecheck if available: `cd apps/digichat && npx tsc --noEmit -p tsconfig.json` (or `npm run build` if that is the project’s type gate). Fix any unused imports in chat-panel.

- [ ] **Step 5: Commit**

```bash
git add apps/digichat/src/lib/chat-activity.ts \
  apps/digichat/src/lib/chat-activity.test.ts \
  apps/digichat/src/components/chat-panel.tsx
git rm apps/digichat/src/components/digigraph-trace.tsx
git commit -m "$(cat <<'EOF'
...
- Modify: `apps/digichat/src/lib/stream-digigraph-trace.ts`
- Modify: `apps/digichat/src/lib/stream-digigraph-trace.test.ts`
- Modify: `apps/digichat/src/app/api/chat/route.ts`
- Modify: `apps/digichat/src/hooks/use-embed-digi-chat.ts` (optional: keep legacy reader for one release — design says delete dual-emit **writer**; embed fallback reader may remain until Phase 3, but prefer updating tests that asserted dual-emit)
- Modify: `apps/digichat/src/hooks/use-embed-digi-chat.test.ts` as needed
...
git add apps/digichat/src/lib/stream-digigraph-trace.ts \
  ...
  apps/digichat/src/hooks/use-embed-digi-chat.test.ts
git commit -m "$(cat <<'EOF'
...
- Create: `apps/digichat/src/lib/digivault-env.ts`
- Create: `apps/digichat/src/lib/digivault-env.test.ts`
...
Run:
```bash
cd apps/digichat && npx vitest run src/lib/embed-tenants.test.ts src/lib/digivault-env.test.ts
```
Expected: FAIL
...
```bash
git add apps/digichat/src/lib/embed-tenants.ts \
  apps/digichat/src/lib/embed-tenants.test.ts \
  apps/digichat/src/lib/digivault-env.ts \
  apps/digichat/src/lib/digivault-env.test.ts
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `apps/digichat/src/lib/digivault-ip-rate-limit.ts`
- Create: `apps/digichat/src/lib/digivault-ip-rate-limit.test.ts`
...
Run: `cd apps/digichat && npx vitest run src/lib/digivault-ip-rate-limit.test.ts`
...
```bash
git add apps/digichat/src/lib/digivault-ip-rate-limit.ts \
  apps/digichat/src/lib/digivault-ip-rate-limit.test.ts
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `apps/digichat/src/lib/digivault-vault.ts`
- Create: `apps/digichat/src/lib/digivault-vault.test.ts`
- Create: `apps/digichat/src/lib/digivault-ndjson-adapter.ts`
- Create: `apps/digichat/src/lib/digivault-ndjson-adapter.test.ts`
- Create fixtures under `apps/digichat/src/lib/fixtures/digivault/`
...
Port `searchVault` / `buildContext` behaviour from `apps/digithings-web/functions/api/chat.ts` (~339–414): POST `${supabaseUrl}/rest/v1/rpc/search_architecture_notes` with `{ query_text, match_count: TOP_K }`, headers `apikey` + `Authorization: Bearer ${anon}`. Truncate `body_markdown` with `MAX_NOTE_CHARS` **only in toolText**. Activity documents: `{ title, path: vault_path }` only.
...
```bash
git add apps/digichat/src/lib/digivault-vault.ts \
  apps/digichat/src/lib/digivault-vault.test.ts \
  apps/digichat/src/lib/digivault-ndjson-adapter.ts \
  apps/digichat/src/lib/digivault-ndjson-adapter.test.ts \
  apps/digichat/src/lib/fixtures/digivault/
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `apps/digichat/src/lib/digivault-byok.ts`
- Create: `apps/digichat/src/lib/digivault-byok.test.ts`
- Modify: `apps/digichat/src/hooks/use-byok-key.ts` (add `gemini`)
- Modify: `apps/digichat/src/components/byok-settings-panel.tsx` and embed BYOK UI provider lists to include `gemini`
...
```bash
git add apps/digichat/src/lib/digivault-byok.ts \
  apps/digichat/src/lib/digivault-byok.test.ts \
  apps/digichat/src/hooks/use-byok-key.ts \
  apps/digichat/src/components/byok-settings-panel.tsx \
...
**Files:**
- Create: `apps/digichat/src/lib/digivault-stream.ts`
- Create: `apps/digichat/src/lib/digivault-stream.test.ts`
...
```bash
git add apps/digichat/src/lib/digivault-stream.ts \
  apps/digichat/src/lib/digivault-stream.test.ts \
  apps/digichat/src/lib/digivault-llm.ts  # if split
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `apps/digichat/src/app/api/chat/route.ts`
- Modify: `apps/digichat/src/app/api/chat/route.test.ts`
- Create/complete: `apps/digichat/src/lib/fixtures/digivault/*`
- Create: `apps/digichat/src/lib/digivault-parity.test.ts`
- Modify: `apps/digichat/ARCHITECTURE.md`
...
cd packages/digichat-ui && npx vitest run
```
...
```bash
git add apps/digichat/src/app/api/chat/route.ts \
  apps/digichat/src/app/api/chat/route.test.ts \
  apps/digichat/src/lib/digivault-parity.test.ts \
  apps/digichat/src/lib/fixtures/digivault/ \
