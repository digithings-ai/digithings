...
> **Superseded 2026-08** by [#1784](https://github.com/digithings-ai/digithings/pull/1784) (Fixes #1760) — applies to the Phase 3 Outcome above and Task 3.3 below: the `[thesisId]` detail route did **not** stay. Thesis detail is now `/portfolio/theses?thesis=<id>`, a single static page reading the id at runtime, with hrefs built by `thesisDetailHref()` in `lib/portfolio-url-state.ts`. Under `output: 'export'` a dynamic segment only pre-builds the ids enumerated at build time and hard-404s on every other one — do **not** reintroduce a `[thesisId]`-style dynamic segment under `/portfolio/theses`. Guarded by `lib/thesis-route-canon.test.ts`.
...
- [ ] **Task 3.3 — Theses tab.** Move the `/portfolio/theses` tracker into the Theses tab (claim, vehicle, status). Keep `/portfolio/theses/[thesisId]` detail. Convert `app/portfolio/theses/page.tsx` to redirect to `/portfolio?tab=theses`. **Test:** Theses tab lists a thesis with its status; `[thesisId]` still renders. Commit. *(Superseded 2026-08 — see the Phase 3 note above: detail is served from `?thesis=<id>`, not a `[thesisId]` segment.)*
