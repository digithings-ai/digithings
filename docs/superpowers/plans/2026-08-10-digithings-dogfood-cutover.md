...
|---|---|---|
| **Pages shell** | `apps/digithings-web/app/chat/page.tsx` → iframe `/embed?host=digithings.ai` | `infra/digichat-digithings/README.md` |
...
| **Embed CSP** | Runtime `DIGICHAT_EMBED_HOSTS` / tenants (after #2031) | `apps/digichat/src/proxy.ts` |
| **Auth** | `/` → `auth()` → redirect `/login`; `/embed` uses tenant `gateMode: ungated` | `apps/digichat/src/app/page.tsx` |
| **digiquant.io** | Marketing/docs site; **no `/chat` page** — corpus crawl target only | `apps/digiquant-web/` |
