...
**Files:**
- Modify: `apps/digichat/src/hooks/use-byok-key.ts` (add type only, no behavior change)
- Create: `apps/digichat/src/hooks/use-byok-key.catalog-parity.test.ts`
...
```ts
// apps/digichat/src/hooks/use-byok-key.catalog-parity.test.ts
...
Run: `cd apps/digichat && npx vitest run src/hooks/use-byok-key.catalog-parity.test.ts`
...
In `apps/digichat/src/hooks/use-byok-key.ts`, add right after the existing `BYOKKeyState` type (currently lines 75-80):
...
```bash
git add apps/digichat/src/hooks/use-byok-key.ts apps/digichat/src/hooks/use-byok-key.catalog-parity.test.ts
...
- Modify: `apps/digichat/src/hooks/use-byok-key.ts` (`BYOKProvider` union, `BYOK_PROVIDER_LIST`, every exhaustive switch)
- Modify: `apps/digichat/src/components/byok-cli-flow.tsx:380-388` (key placeholder ternary)
- Modify: `apps/digichat/src/app/api/byok/test/route.ts` (`BYOKProvider` type, `readProvider`, `testKey` switch, new `testXaiKey`)
- Modify: `apps/digichat/src/app/api/chat/route.ts` (`byokNeedsModel` OR-chain)
- Modify: `tests/dg/test_llm_auth.py` (the 4 parametrize lists that currently treat `"xai"` as unrouted)
- Test: `apps/digichat/src/hooks/use-byok-key.test.ts`, `apps/digichat/src/app/api/byok/test/route.test.ts`
...
In `apps/digichat/src/hooks/use-byok-key.ts`:
...
In `apps/digichat/src/components/byok-cli-flow.tsx`, change (currently lines 380-388):
...
In `apps/digichat/src/app/api/byok/test/route.ts`:
...
Add to `apps/digichat/src/app/api/byok/test/route.test.ts`, after the existing Gemini tests:
...
In `apps/digichat/src/app/api/chat/route.ts` (currently lines 203-206):
...
git add config/byok-providers.json \
  ...
  apps/digichat/src/components/byok-cli-flow.tsx \
  apps/digichat/src/app/api/byok/test/route.ts \
  apps/digichat/src/app/api/byok/test/route.test.ts \
...
**Files:**
- Create: `apps/digichat/src/lib/fetch-with-timeout.ts`
- Create: `apps/digichat/src/lib/fetch-with-timeout.test.ts`
- Modify: `apps/digichat/src/app/api/byok/test/route.ts` (import instead of local copies — behavior-preserving)
...
```ts
// apps/digichat/src/lib/fetch-with-timeout.test.ts
...
Run: `cd apps/digichat && npx vitest run src/lib/fetch-with-timeout.test.ts`
Expected: FAIL — `Error: Cannot find module './fetch-with-timeout'`

- [ ] **Step 3: Implement**

```ts
// apps/digichat/src/lib/fetch-with-timeout.ts
...
In `apps/digichat/src/app/api/byok/test/route.ts`, delete the local `TIMEOUT_MS` constant, `fetchWithTimeout` function, and `abortOrMessage` function (their current bodies), and add an import:
...
```bash
git add apps/digichat/src/lib/fetch-with-timeout.ts \
  apps/digichat/src/lib/fetch-with-timeout.test.ts \
  apps/digichat/src/app/api/byok/test/route.ts
...
**Files:**
- Modify: `apps/digichat/src/app/api/byok/test/route.ts`
- Test: `apps/digichat/src/app/api/byok/test/route.test.ts`
...
Add to `apps/digichat/src/app/api/byok/test/route.test.ts`:
...
In `apps/digichat/src/app/api/byok/test/route.ts`, change the shared type:
...
```bash
git add apps/digichat/src/app/api/byok/test/route.ts apps/digichat/src/app/api/byok/test/route.test.ts
...
**Files:**
- Create: `apps/digichat/src/lib/openrouter-catalog.ts`
- Create: `apps/digichat/src/lib/openrouter-catalog.test.ts`
...
```ts
// apps/digichat/src/lib/openrouter-catalog.test.ts
import { describe, expect, it } from "vitest";
...
Run: `cd apps/digichat && npx vitest run src/lib/openrouter-catalog.test.ts`
Expected: FAIL — `Cannot find module './openrouter-catalog'`

- [ ] **Step 3: Implement**

```ts
// apps/digichat/src/lib/openrouter-catalog.ts
...
```bash
git add apps/digichat/src/lib/openrouter-catalog.ts apps/digichat/src/lib/openrouter-catalog.test.ts
...
**Files:**
- Create: `apps/digichat/src/app/api/byok/models/route.ts`
- Create: `apps/digichat/src/app/api/byok/models/route.test.ts`
...
```ts
// apps/digichat/src/app/api/byok/models/route.test.ts
...
Run: `cd apps/digichat && npx vitest run src/app/api/byok/models/route.test.ts`
Expected: FAIL — `Error: Cannot find module './route'`

- [ ] **Step 3: Implement**

```ts
// apps/digichat/src/app/api/byok/models/route.ts
import { requireDigiChatAuth } from "@/lib/request-auth";
...
```bash
git add apps/digichat/src/app/api/byok/models/route.ts apps/digichat/src/app/api/byok/models/route.test.ts
...
**Files:**
- Modify: `apps/digichat/src/hooks/use-byok-key.ts`
- Test: `apps/digichat/src/hooks/use-byok-key.test.ts`
...
Add to `apps/digichat/src/hooks/use-byok-key.test.ts`:
...
Run: `cd apps/digichat && npx vitest run src/hooks/use-byok-key.test.ts -t "preference cookie"`
Expected: FAIL — `readByokPrefCookie is not exported`

- [ ] **Step 3: Implement**

Add to `apps/digichat/src/hooks/use-byok-key.ts`:
...
```bash
git add apps/digichat/src/hooks/use-byok-key.ts apps/digichat/src/hooks/use-byok-key.test.ts
...
At the top of `apps/digichat/src/components/byok-cli-flow.tsx`, add:
...
```bash
git add apps/digichat/src/components/byok-cli-flow.tsx
...
**Files:**
- Create: `apps/digichat/src/components/byok-cli-flow.test.tsx`
...
Run: `grep -n "@testing-library/react" apps/digichat/package.json`
If absent, this repo may test components differently — check an existing `*.render.test.tsx` file (e.g. `packages/digichat-ui/src/DigiChatSession.render.test.tsx`) for the actual rendering/query utilities already in use in this monorepo, and match that pattern instead of assuming `@testing-library/react` — adjust the imports below accordingly before writing Step 2.

- [ ] **Step 2: Write the test**

```tsx
// apps/digichat/src/components/byok-cli-flow.test.tsx
...
Run: `cd apps/digichat && npx vitest run src/components/byok-cli-flow.test.tsx`
...
```bash
git add apps/digichat/src/components/byok-cli-flow.test.tsx
...
- [ ] **Step 1: Update `apps/digichat/ARCHITECTURE.md`**

Run: `grep -n "BYOK providers listed" apps/digichat/ARCHITECTURE.md` to find the current prose list of providers, and update it to name all 5 (openai, openrouter, anthropic, gemini, xai) and note the catalog file as the source of truth:
...
```bash
git add apps/digichat/ARCHITECTURE.md digigraph/ARCHITECTURE.md
...
```bash
cd apps/digichat && npm run test && npm run lint && npm run build
