...
## Frontend (apps/digichat) — shared language list

### Task 4: Curated language list + browser detection helpers

**Files:**
- Create: `apps/digichat/src/lib/languages.ts`
- Test: `apps/digichat/src/lib/languages.test.ts`
...
```typescript
// apps/digichat/src/lib/languages.test.ts
...
Run: `cd apps/digichat && npx vitest run src/lib/languages.test.ts`
Expected: FAIL with "Cannot find module '@/lib/languages'"

- [ ] **Step 3: Write minimal implementation**

```typescript
// apps/digichat/src/lib/languages.ts
/**
...
```bash
git add apps/digichat/src/lib/languages.ts apps/digichat/src/lib/languages.test.ts
...
**Files:**
- Modify: `apps/digichat/src/lib/embed-tenants.ts` (`EmbedTenantConfig` type + `validateEntry`)
- Modify: `apps/digichat/src/lib/embed-client-config.ts` (`EmbedTenantClientConfig`, `toEmbedClientConfig`, `DEFAULT_EMBED_TENANT_CONFIG`)
- Modify: `apps/digichat/src/lib/embed-ui-flags.ts` (`resolveEmbedUiFlags`)
- Modify: `apps/digichat/src/lib/embed-tenants.test.ts`, `apps/digichat/src/lib/embed-ui-flags.test.ts`
...
Append to `apps/digichat/src/lib/embed-tenants.test.ts` (mirror the existing `showByok` test cases in that file):
...
Add a new test file section (or append if a `toEmbedClientConfig` test already exists in `embed-tenants.test.ts` — check first; if not, add to `apps/digichat/src/lib/embed-client-config.test.ts`, creating it if it does not exist):

```typescript
// apps/digichat/src/lib/embed-client-config.test.ts
...
Append to `apps/digichat/src/lib/embed-ui-flags.test.ts`:
...
Run: `cd apps/digichat && npx vitest run src/lib/embed-tenants.test.ts src/lib/embed-client-config.test.ts src/lib/embed-ui-flags.test.ts`
...
In `apps/digichat/src/lib/embed-tenants.ts`, add to the `EmbedTenantConfig` type, next to `showByok`:
...
In `apps/digichat/src/lib/embed-client-config.ts`, add to `EmbedTenantClientConfig`:
...
In `apps/digichat/src/lib/embed-ui-flags.ts`, extend the return type and object of `resolveEmbedUiFlags`:
...
git add apps/digichat/src/lib/embed-tenants.ts apps/digichat/src/lib/embed-tenants.test.ts \
        apps/digichat/src/lib/embed-client-config.ts apps/digichat/src/lib/embed-client-config.test.ts \
        apps/digichat/src/lib/embed-ui-flags.ts apps/digichat/src/lib/embed-ui-flags.test.ts
...
**Files:**
- Create: `apps/digichat/src/components/language-select.tsx`
- Test: `apps/digichat/src/components/language-select.test.tsx`
...
```typescript
// apps/digichat/src/components/language-select.test.tsx
...
Run: `cd apps/digichat && npx vitest run src/components/language-select.test.tsx`
Expected: FAIL with "Cannot find module '@/components/language-select'"

- [ ] **Step 3: Write minimal implementation**

```tsx
// apps/digichat/src/components/language-select.tsx
"use client";
...
```bash
git add apps/digichat/src/components/language-select.tsx apps/digichat/src/components/language-select.test.tsx
...
- Modify: `apps/digichat/src/hooks/use-embed-digi-chat.test.ts`
...
Read `apps/digichat/src/hooks/use-embed-digi-chat.test.ts` first to match its existing test harness for asserting on `prepareSendMessagesRequest`-produced headers (it already has cases for `X-BYOK-Key` — follow that exact pattern for the transport mock/assertion style). Add:
...
Run: `cd apps/digichat && npx vitest run src/hooks/use-embed-digi-chat.test.ts`
Expected: FAIL — `X-Digi-Language` header is never set.

- [ ] **Step 3: Write minimal implementation**

In `apps/digichat/src/hooks/use-embed-digi-chat.ts`, add `responseLanguage?: string` to `UseEmbedDigiChatOptions` and destructure it in `useEmbedDigiChat`'s parameter list, then inside `prepareSendMessagesRequest`, next to the `byokKey` block:
...
```bash
git add apps/digichat/src/hooks/use-embed-digi-chat.ts apps/digichat/src/hooks/use-embed-digi-chat.test.ts
...
**Files:**
- Modify: `apps/digichat/src/app/embed/embed-client.tsx` (`EmbedChat`)
...
Run: `cd apps/digichat && npm run dev`

Open `http://localhost:3000/embed?host=digithings.ai` (or whatever local embed URL this repo's dev docs use for a first-party host — check `apps/digichat/OPERATIONS.md` if unsure of the exact query params for local first-party embed testing).
...
```bash
git add apps/digichat/src/app/embed/embed-client.tsx
...
Run: `cd apps/digichat && npx vitest run src/app/api/chat/route.test.ts`
...
Run: `cd apps/digichat && npx vitest run`
...
```bash
git add apps/digichat/src/app/api/chat/route.ts apps/digichat/src/app/api/chat/route.test.ts
...
**Files:**
- Modify: `apps/digichat/src/lib/adapters/foundry/stream.ts`
- Modify: `apps/digichat/src/lib/adapters/foundry/stream.test.ts`
- Modify: `apps/digichat/ARCHITECTURE.md` (document the dual-backend language contract)
...
Run: `cd apps/digichat && npx vitest run src/lib/adapters/foundry/stream.test.ts`
...
In `apps/digichat/src/lib/adapters/foundry/stream.ts`, add a pure helper near the top (below the imports, alongside the other small pure helpers like `stripFoundryCitationMarkers`), using the `LANGUAGES` array from Task 4 (the frontend list is an array of `{code, label}`, not a map — unlike the Python-side `LANGUAGE_NAMES` from Task 1):
...
Run: `cd apps/digichat && npm run lint`
