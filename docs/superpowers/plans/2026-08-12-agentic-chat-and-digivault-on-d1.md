...
- **Two databases, never one shared table with a filter.** `digithings_docs` and `occ_help`, matching `DIGI_TENANT_CORPUS_MAP` in `apps/digithings-stack-cloudflare/wrangler.toml:85` and the two existing Vectorize index names.
...
| `apps/digithings-stack-cloudflare/src/index.ts` | `D1_ACCOUNT_ID`, `D1_API_TOKEN`, `D1_DATABASE_MAP` in `envVars` **and** `Env`. |
| `apps/digithings-stack-cloudflare/container/entrypoint.sh` | Export the three D1 vars. `DIGIVAULT_ROOT` handling is deliberately untouched. |
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "D1_DATABASE_MAP (see apps/digithings-stack-cloudflare/wrangler.toml); "
...
**Files:**
- Modify: `apps/digithings-stack-cloudflare/src/index.ts:41-72` and `:112-139`
- Modify: `apps/digithings-stack-cloudflare/container/entrypoint.sh`
- Modify: `.github/workflows/docs-onboard-digithings.yml:144-187`
- Test: `apps/digithings-stack-cloudflare/src/ports.test.ts`
...
Run: `cd apps/digithings-stack-cloudflare && npm run typecheck && npm test`
Expected: PASS

- [ ] **Step 6: Commit**

```bash
git add apps/digithings-stack-cloudflare/src/index.ts apps/digithings-stack-cloudflare/container/entrypoint.sh .github/workflows/docs-onboard-digithings.yml
...
```bash
cd apps/digithings-stack-cloudflare
