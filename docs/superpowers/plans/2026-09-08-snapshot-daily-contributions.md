...
**Files:**
- Modify: `packages/ui/src/components/repo-activity/types.ts`
- Modify: `packages/ui/src/components/repo-activity/RepoActivity.tsx`
...
Run: `npm test -- src/components/repo-activity` (from `packages/ui`)
Expected: 5 files / 19+ pass; `demo.ts` without the field still renders (fallback path).

- [ ] **Step 4: Commit**

```bash
git add packages/ui/src/components/repo-activity/types.ts packages/ui/src/components/repo-activity/RepoActivity.tsx
...
- Regenerate: `apps/digithings-web/lib/repo-activity.json`
- Modify: `packages/ui/src/components/repo-activity/demo.ts`
...
- [ ] **Step 3: synthetic series for `demo.ts`** — a tiny `dailyContributions` (e.g. last 14 days nonzero) so the reference specimen shows graded cells without bloating the fixture; then `npm test -- src/components/repo-activity` (from `packages/ui`) green.

- [ ] **Step 4: Commit** (JSON + script + demo together — the bare `--check` in `test-web.yml` must see the key in the same PR that requires it)

```bash
git add scripts/fetch_repo_activity.py tests/scripts/test_fetch_repo_activity.py apps/digithings-web/lib/repo-activity.json packages/ui/src/components/repo-activity/demo.ts
