...
| `apps/digichat/OPERATIONS.md` | Link install guide; clarify GHCR vs local build |
| `docs/architecture/digichat-modular-frontend.md` §5 | Link to INSTALL.md |
| `apps/digichat/embed-hosts.txt` | Documented rebuild path; optional client host notes |
...
- Modify: `apps/digichat/OPERATIONS.md` (add short “Release artifacts” subsection linking RELEASE-SMOKE + INSTALL)

**Interfaces:**
- Consumes: Existing workflows `release-please-digichat.yml` (tag `digichat-vX.Y.Z` on develop) and `publish-digichat-image.yml` (GHCR on main). Current app version `0.9.3` in `apps/digichat/package.json`.
...
```bash
jq -r .version apps/digichat/package.json
jq -r .private apps/digichat/package.json
test -f .github/workflows/release-please-digichat.yml
test -f .github/workflows/publish-digichat-image.yml
head -20 apps/digichat/CHANGELOG.md
```
...
At the top of `apps/digichat/OPERATIONS.md` (after the title / intro), add:
...
rg -n "Release artifacts" apps/digichat/OPERATIONS.md
```
...
```bash
git add docs/digichat/RELEASE-SMOKE.md apps/digichat/OPERATIONS.md
...
- Modify: `apps/digichat/OPERATIONS.md` (replace stub link if Task 1 left a forward ref)
...
6. **Embed CSP note:** stock GHCR uses `apps/digichat/embed-hosts.txt`; new parent hosts need rebuild (Task 7) until runtime CSP exists.
...
- **Marketing shell:** `digithings.ai` — Cloudflare Pages (`apps/digithings-web/`).
...
- Modify: `apps/digichat/ARCHITECTURE.md` (Environment variables / embed hosts — “client rebuild” note)
- Modify: `apps/digichat/embed-hosts.txt` (header comment: how clients add hosts)
- Optional doc-only: `docs/digichat/EMBED-HOSTS-REBUILD.md` if INSTALL would get too long

**Interfaces:**
- Consumes: Build-arg `DIGICHAT_EMBED_HOSTS` in `apps/digichat/Dockerfile` and publish workflow reading `embed-hosts.txt`. CSP evaluated at `next build` via `next.config.ts` importing `security-headers.ts`.
...
`apps/digichat/embed-hosts.txt` at build time. If your parent site hostname
...
rg -n "DIGICHAT_EMBED_HOSTS" apps/digichat/Dockerfile
```

- [x] **Step 5: Commit**

```bash
git add docs/digichat/INSTALL.md apps/digichat/embed-hosts.txt apps/digichat/ARCHITECTURE.md
git commit -m "$(cat <<'EOF'
