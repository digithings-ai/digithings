...
Browser digithings.ai/chat
  → apps/digithings-web/app/chat/page.tsx (DtNav + iframe)
...
References: [`infra/digichat-digithings/README.md`](../../../infra/digichat-digithings/README.md), [`docs/adr/0018-digichat-path-routing.md`](../../adr/0018-digichat-path-routing.md), [`apps/digithings-web/components/ChatEmbedShell.tsx`](../../../apps/digithings-web/components/ChatEmbedShell.tsx).
...
1. **Pages:** Add `apps/digithings-web/app/chat/occ/page.tsx` reusing `ChatEmbedShell` with `embedHostKey="occ.digithings.ai"` (new prop) instead of hardcoded `digithings.ai`.
...
3. **First-party allowlist:** Extend `FIRST_PARTY_EMBED_HOSTS` in `apps/digichat/src/lib/embed-first-party.ts` with `occ.digithings.ai` so the iframe works without `?token=` (same pattern as #1866 for `digithings.ai`).
...
|-----------|-------------------|
| `apps/digichat` `EmbedBackendConfig` | Optional `digisearchIndex`, `vaultPathPrefix` on `digigraph` backend |
| `apps/digichat` `/api/chat` | Forward `X-Digi-Corpus-Index` / `X-Digi-Vault-Prefix` when set on tenant backend |
...
| [`apps/digichat/ARCHITECTURE.md`](../../../apps/digichat/ARCHITECTURE.md) | Embed tenant registry |
