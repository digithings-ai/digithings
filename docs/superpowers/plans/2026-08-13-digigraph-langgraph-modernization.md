...
def test_tool_role_messages_are_silently_omitted_today() -> None:
    ...
    construct such a message lives in apps/digichat/src/lib/adapters/digithings/,
...
```python
    ...
    - ``role="tool"`` turns are also omitted, today deliberately: digichat's OpenAI-compat
      ...
      under ``apps/digichat/src/lib/adapters/digithings/``). If a caller ever DOES
