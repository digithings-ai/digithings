...
| `chrome.transcript.userAlign` | **Documented, not configurable.** The first-party `digichat` skin forces left alignment regardless of the value, so exposing it on the embed would be a no-op. |
