...
| **3** | [`2026-08-09-digichat-corpus-ingest.md`](../superpowers/plans/2026-08-09-digichat-corpus-ingest.md) | Ship client-side crawl/PDF ingest (`digicorpus`) into **their** digivault so Profile A digichat → digigraph → `digivault_search_notes` grounds answers on that client's docs. |

Sketch gaps these close (from [`digichat-self-hosted-release.md`](digichat-self-hosted-release.md) §5 Still open): runtime CSP `frame-ancestors`; GHCR for digikey / digigraph / digivault; corpus / crawl / OCR / vault ingest.
...
| **Pick 3 after digivault is reachable under Profile A** | Ingest writes `DIGIVAULT_ROOT` / uses `DIGIVAULT_URL`; end-to-end chat smoke needs Pick 2’s stack. digivault **local search** (Pick 3 Task 1) can start earlier against a local digivault, but client doc-chatbot acceptance assumes Profile A. |

```text
Pick 2 T1 (publish GHCR) ──┬── Pick 2 T2–4 (Profile A pull + config)
Pick 1 (runtime CSP)  ─────┘         │
                                     ▼
                              Pick 3 MVP (local search → digicorpus → runbook)
```
...
| `DIGIVAULT_ROOT` | digivault (+ digicorpus CLI) | Vault filesystem root / Compose volume target for ingest + local search |
...
| Artifact | Contract |
|---|---|
| Compose volume `digivault_data` → `/data/vault` | Stable across GHCR digivault upgrades; Pick 3 writes here (or via `POST /v1/notes`), never into digichat. |
...
| digicorpus | Offline job/CLI **beside** the stack — not a Compose chat-tier service. |

### digivault URL / root

- digigraph keeps `DIGIVAULT_URL=http://digivault:8004` on Profile A.
- Pick 3: when `DIGIVAULT_ROOT` is set, `digivault_search_notes` searches the local vault; otherwise Supabase FTS (digithings.ai reference). Same volume digicorpus writes.
...
### Stage D — Corpus MVP (Pick 3)

- [ ] digivault local filesystem search when `DIGIVAULT_ROOT` set (`digivault_search_notes`)
- [ ] `digicorpus` scaffold + note writer (Vault upsert; no digichat imports)
- [ ] HTML crawl → page notes; PDF text → page notes (`[pdf]` extra)
- [ ] `docs/digichat/CORPUS-INGEST.md` + INSTALL pointer; mark sketch corpus follow-up
- [ ] Profile A smoke: ingest into volume → search / digichat question hits ingested phrase

### Stage E — Later (out of MVP for this synthesis)

- [ ] Pick 3 Phase 2: OCR / images / base64 assets
