...
`apps/digithings-stack-cloudflare/container/seed_chroma.sh`). digisearch
...
- If the Vectorize leg (`apps/digithings-stack-cloudflare/container/entrypoint.sh`,
...
- **Cutover is executed through a shared, non-isolated config surface --
  ...
  named it.** `apps/digithings-stack-cloudflare/wrangler.toml`'s
