...
**Historical record.** The `chat.digithings.ai` subdomain target described below (ADR-0002) was later superseded by [ADR-0018](../adr/0018-digichat-path-routing.md), which routes digichat at `digithings.ai/chat`. `cloudflare/digichat/` is not deployed anywhere today — see epic [#1248](https://github.com/digithings-ai/digithings/issues/1248) for the in-progress cutover to the real routing target.
