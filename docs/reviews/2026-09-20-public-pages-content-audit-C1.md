...
| `apps/digithings-web/app/quality/page.tsx` (limits row) | rewrote "Frontend is scored differently" — **then corrected again, because the rewrite was wrong** | The audit reasoned from the *tool* (`score.py` applies a `.py` filter, so its pandas/polars patterns only see Python) rather than from the *job*: the CI score job excludes `apps/**` and `packages/**` **by pathspec** (`.github/workflows/test-score.yml:55`), and `scripts/ci_paths.yaml` documents exactly that. So the **original sentence was true**, and the replacement — "the score job only reads Python diffs, so the rubrics never see JS or CSS" — was **false**: `score.py` scans every added line for the non-pandas patterns, so it does flag JS/CSS, and `scripts/*.mjs` is not excluded by the job at all. The true mechanism has been restored. Separately, the "design reference" half of the exceptions sentence went stale within the same window: #4445 added a `Test design-reference` step to `test-web.yml`, so **digiquant-web is now the only exception** (its suite runs in no lane — see the follow-up issue). |
