...
| Core service trees | **Keep** — all six Python components + `cloudflare/*` active |
| Local-only artifacts (OK) | `dist/`, `digiquant/data/*.csv`, root `data/price-history/`, `node_modules/` |
| Committed artifacts to remove | `cloudflare/olympus/public/dashboard-data.json` (~284 KB), `digiquant/tearsheet.html` (~1.3 MB) |
| Stale references | `apps/digiquant-atlas/`, `cloudflare/atlas/` in lockfile + docs; `website/` in DEPLOYMENT |
...
| `cloudflare/` | Web umbrella: design, landings, digichat, Olympus | npm workspaces, Cloudflare/build scripts | **Keep** | High |
...
| `package.json` + `package-lock.json` | npm workspaces `cloudflare/*` | CI, local frontend builds | **Keep** lockfiles; **regenerate** to drop stale `apps/digiquant-atlas` entries (JAN-003) | High |
...
**No `apps/` directory** exists today. Atlas lives under `digiquant/src/digiquant/olympus/atlas/`; UI at `cloudflare/olympus/`.
...
|---------|------|--------------|-------|
| `cloudflare/digiweb/design/` | npm workspace `@digithings/design` | Imported by digichat, olympus, static sites | **Keep** |
| `cloudflare/digithings/` | Static HTML | Cloudflare / `build-digithings.sh` | **Keep** |
| `cloudflare/digiquant/` | Static HTML + `atlas.html` | `build-digiquant.sh`; ADR-0012 publish repo | **Keep** |
| `cloudflare/digichat/` | Next.js 16 BFF + chat | Compose profile `digichat` | **Keep** |
| `cloudflare/olympus/` | Next.js dashboard (`basePath: /olympus`) | Built into `dist/olympus/`; Supabase at runtime | **Keep** |

**Olympus artifact:** `cloudflare/olympus/public/dashboard-data.json` (~284 KB) **is committed**. Atlas ops docs claim it is gitignored — **doc is wrong**. Aligns with **AUDIT-037 / REM-037** (JAN-001).
...
|------|------:|-------|----------------|
| `cloudflare/olympus/public/dashboard-data.json` | 284 KB | Portfolio static fallback shipped as public asset | **AUDIT-037**, **REM-037**, JAN-001 |
| `digiquant/tearsheet.html` | 1.3 MB | Generated Plotly HTML sample in component tree | JAN-002 |
| `package-lock.json` (root) | 519 KB | Stale `apps/digiquant-atlas/frontend` + `cloudflare/atlas` extraneous packages | JAN-003 |
...
|-----------------|-------|-----------------|
| `apps/digiquant-atlas/` | `docs/plans/`, ADRs 0009–0014, backlog-reshape | `digiquant/src/digiquant/olympus/atlas/`, `cloudflare/olympus/` |
| `cloudflare/atlas/` | Root `package-lock.json` | `cloudflare/olympus/` |
| `website/` | `docs/DEPLOYMENT.md`, `docs/VISION.md` | `cloudflare/digithings/` |
...
|----|--------|--------|--------|------|
| **JAN-001** | Remove from git + pipeline | `cloudflare/olympus/public/dashboard-data.json` | S | Low — **REM-037** |
| **JAN-002** | Gitignore + stop committing | `digiquant/tearsheet.html` | S | Low |
| **JAN-003** | Regenerate lockfile | Root `package-lock.json` drop `apps/digiquant-atlas`, `cloudflare/atlas` | S | Low |
...
| **JAN-007** | Gitignore | `cloudflare/olympus/public/dashboard-data.json` after removal | S | Low |
| **JAN-008** | Doc sweep | Replace `apps/digiquant-atlas` in `docs/plans/` + ADRs (historical banners) | M | Low |
| **JAN-009** | Doc fix | `docs/DEPLOYMENT.md`: `website/` → `cloudflare/digithings/` | S | Low |
...
| **JAN-031** | Keep | `cloudflare/digiweb/design` + static landings | — | — |
...
| `cloudflare/*` | All active; olympus = ex-atlas UI |
