...
| Artifact | Purpose |
|----------|---------|
| `apps/dashboard/app/api/snapshots/route.ts` | `GET /api/snapshots` (service role) |
| `apps/dashboard/lib/snapshot-fetch.ts` | `NEXT_PUBLIC_DASHBOARD_USE_BFF=1` → fetch BFF |
| `apps/dashboard/examples/bff-snapshots-route.example.ts` | Mirror of route for static-export docs |
| `digiquant/supabase/migrations/028_olympus_bff_notes.sql` | BFF migration notes |
| `apps/dashboard/README.md` | Threat model + env table |
