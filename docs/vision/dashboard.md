...
The dashboard frontend (`apps/dashboard`) for the finance sub-graphs that live inside digiquant as `digiquant.dashboard`. It turns pipeline output into a navigable, daily decision surface rather than raw research dumps: a "Morning Read" overview, surfaced bull/bear theses and risk debate, portfolio/NAV tracking, and entry points into strategy work.
