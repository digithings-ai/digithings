...
8. **CLI (optional)** — `apps/digichat/cli` with `local-cli.yaml` (`cli.enabled: true`) hits the same `POST /api/chat`. Confirm Next/product sources still do not import `ink` / `@assistant-ui/react-ink` (`cli-isolation` unit test).
