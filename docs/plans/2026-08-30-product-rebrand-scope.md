...
|------|-----------|----------------|
| **olympus** | Product brand for the operator dashboard (`apps/dashboard`, `digiquant.io/olympus/`) and the Python umbrella (`digiquant.olympus`) | Yes — nav CTA, page title, OAuth app, Access path |
...
The dashboard **already** uses functional nav: Brief, Portfolio, Pipeline, FX Hub (`apps/dashboard/lib/nav.ts`). Atlas / Hermes / Kairos are marketing and code names, not chrome labels. Stripping subsystem brands from the UI is mostly a landing-page + copy job.
...
```
apps/dashboard/public/icons/dashboard-app-dark.svg
apps/digiquant-web/components/landing/OlympusMark.tsx
```

Existing marketing already uses the *box* metaphor. From `packages/design/COPY_GUIDE.md`:
...
| Page title / PWA name | `Olympus — digiquant` | `apps/dashboard/app/layout.tsx` |
...
| CSP / `_headers` | scoped to `/olympus*` | `apps/dashboard/lib/security-headers.mjs` |
...
| npm | workspace folder `apps/dashboard`, package name `dashboard` | Shipped with wave 3. |
...
| **3** | `apps/dashboard` folder + npm workspace name. Keep `.oly-*` CSS. | Med | one-hop `develop` |
...
- Wave 1–3: `cd apps/dashboard && npm run test && npm run build`; landing copy grep; `make doc-check`
