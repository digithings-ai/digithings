...
- **dashboard** — the human-facing dashboard (`apps/dashboard`) for digiquant's finance sub-graph trio. research now lives inside `digiquant` as `digiquant.dashboard` (see ADR-0014, ADR-0015); the three sub-graphs are:
...
```
    ...
    digiquant.io ────▶  digiquant product UI
       dashboard    ────▶  research + portfolio + execution dashboard (apps/dashboard)
