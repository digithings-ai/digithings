...
| `CLOUDFLARE_EMAIL_API_TOKEN` · `CLOUDFLARE_ACCOUNT_ID` · `NOTIFY_FROM` | `execution-cron-check.yml:45-47`; `digiquant/src/digiquant/notify/cloudflare_email.py`; `digiquant/.../staging_secrets.py` | `CLOUDFLARE_ACCOUNT_ID` only (repo secret); the token and `NOTIFY_FROM` are at no level yet (2026-09-18) — `unresolved` | no — write-only | digest email stops; the daily probe fails closed (exit 2) | GH + `.env` | replaced Mailgun in #4358; the token needs **Email Sending: Edit**, so do not reuse the deploy token |
...
- **Every level is enumerable now** (repo secret/variable, org secret, `production` environment) — see [Storage surfaces](#storage-surfaces) and `make secrets-audit`, which reports a read it cannot place at any level as `unresolved`. Two are open, both in `execution-cron-check.yml` and both deliberate: `CLOUDFLARE_EMAIL_API_TOKEN` and `NOTIFY_FROM` — the daily probe fails closed (exit 2) exactly to say that digest email is not wired, and Actions substitutes an empty string for each. `CLOUDFLARE_ACCOUNT_ID` is already a repo secret. #4357 resolved the other three: `E2E_BEARER_TOKEN` is now minted in-workflow from the compose stack, the OpenWiki step reads the org-level `LANGSMITH_API_KEY`, and the `docs-reindex-guide.yml` apply step that read `DIGISEARCH_URL` was removed (it posts a local filesystem path with no auth). #4358 replaced the Mailgun sender with Cloudflare Email Sending, which needs digiquant.io onboarded and a token with **Email Sending: Edit** — the only owner action left.
