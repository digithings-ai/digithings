...
- **Every level is enumerable now** (repo secret/variable, org secret, `production` environment) — see [Storage surfaces](#storage-surfaces) and `make secrets-audit`, which reports a read it cannot place at any level as `unresolved`. Three are open, all in `execution-cron-check.yml` and all deliberate: `MAILGUN_API_KEY`, `MAILGUN_DOMAIN`, `NOTIFY_FROM` — the daily probe fails closed (exit 2) exactly to say that digest email is not wired, and Actions substitutes an empty string for each. #4357 resolved the other three: `E2E_BEARER_TOKEN` is now minted in-workflow from the compose stack, the OpenWiki step reads the org-level `LANGSMITH_API_KEY`, and the `docs-reindex-guide.yml` apply step that read `DIGISEARCH_URL` was removed (it posts a local filesystem path with no auth). #4358 migrates the Mailgun sender to Cloudflare Email Sending, which is where the three names will be replaced.
