...
- **GitHub repo secret/var set.** Enumerable at repo level since 2026-09-18 — see [Storage surfaces](#storage-surfaces) for the 14 secrets and 11 variables, and `make secrets-audit` for the drift check. Org-level and environment secrets are still not enumerable here (`gh secret list --org` needs `admin:org`; the `production` environment holds 3 names), so a read the tool reports as `not repo-level` usually resolves at one of those levels — but it can equally be a typo, in which case the workflow silently receives an empty string.
