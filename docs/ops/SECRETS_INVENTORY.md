...
**GitHub repo secret / var.** Repo-scoped by default. Only 4 of 72 workflows declare `environment: production` (`deploy-digithings-stack-cloudflare.yml:79`, `db-migrate.yml:105`, `sync-architecture-vault.yml:40`, `docs-onboard-digithings.yml:144`); every other `secrets.*` / `vars.*` read has no deployment gate.
...
| `DIGIKEY_PRIVATE_KEY_PEM` | `cloudflare/digithings-stack-cloudflare/src/index.ts:67`; `digikey/src/digikey/crypto_keys.py:61` | `cloudflare/digithings-stack-cloudflare/wrangler.toml:137` | no — write-only | every minted JWT + JWKS; org-wide 401 | GH repo secret; password manager | RS256, static `kid=digikey-1`, no overlap |
...
| `DIGITHINGS_PROJECT_TOKEN` | 13 workflows (`agent-backlog-snapshot.yml:28` … `project-status.yml:112`) | GH repo secret | n/a (GH secret) | GitHub Projects v2 / GraphQL automation | widest CI token | see R13 |
...
| `E2E_BEARER_TOKEN` | `.github/workflows/test-e2e.yml:67` | GH repo secret | n/a | mock e2e fails | CI only | test fixture |

### (b) provider / LLM API keys

| Name | Consumer (file:line) | Defined in (file:line) | Readback? | Rotation blast radius | Duplicate copies | Notes |
|---|---|---|---|---|---|---|
| `OPENROUTER_API_KEY` | `digillm/src/digillm/client.py:242`; stack `src/index.ts:101` | GH repo secret; stack `wrangler.toml:145` | no — write-only | house LLM routing 401 | GH + stack Worker + digichat Worker (**dead**) + local `.env` | triplicated; see R4 |
...
| `GROQ_API_KEY` | stack `src/index.ts:100`; GH workflows | GH repo secret; stack `wrangler.toml:145` | no — write-only | digigraph/LiteLLM calls 401 | GH + stack Worker + `.env` | |
| `OPENAI_API_KEY` | stack `src/index.ts:102`; `digigraph/src/digigraph/model_config.py:510` | GH repo secret; `wrangler.toml:145` | no — write-only | OpenAI models + embeddings 401 | GH + stack (documented) + `.env` | absent from live stack set (R9) |
...
| `MISTRAL_API_KEY` · `CEREBRAS_API_KEY` · `DEEPSEEK_API_KEY` · `NVIDIA_API_KEY` | `pipeline-provider-review.yml:92-97` | GH repo secret | n/a | provider-review job probes fail | CI only | not used by services |
| `LANGSMITH_API_KEY` · `OPENWIKI_LANGSMITH_API_KEY` | `digismith/src/digismith/config.py:27`; `openwiki-update.yml:73` | GH repo secrets | n/a | tracing export stops | GH only | distinct key for OpenWiki runs |
| `FRED_API_KEY` | stack `src/index.ts:180`; `digiquant/src/digiquant/cli/prices.py:519` | GH repo secret; `wrangler.toml:151`; committed example | no — write-only | macro/market-data reads fail | GH + stack (documented) + committed example | absent live (R9); `plaintext-literal` in example |
...
| `COINGECKO_API_KEY` · `ALPHA_VANTAGE_API_KEY` · `SEC_EDGAR_USER_AGENT` | `digiquant/.../research ingest` | `digiquant/src/digiquant/research/config/mcp.secrets.env.example:5-12` | n/a | research ingest fails | committed example, gitleaks-allowlisted | `plaintext-literal`; owner-confirmed dead 2026-06-18 |
...
| Name | Consumer (file:line) | Defined in (file:line) | Readback? | Rotation blast radius | Duplicate copies | Notes |
|---|---|---|---|---|---|---|
| `CLOUDFLARE_API_TOKEN` · `VECTORIZE_API_TOKEN` · `D1_API_TOKEN` | stack `src/index.ts:89-93`; `digivault/src/digivault/server.py:171`; `scripts/d1_sync.py:442` | stack `wrangler.toml:159,190-193`; GH repo secret | no — write-only | Vectorize + D1 sync; deploy workflows | stack Worker (canonical + legacy), GH, local `.env` | **also wrangler's own auth var** — see R7 |
| `CLOUDFLARE_ACCOUNT_ID` · `VECTORIZE_ACCOUNT_ID` · `D1_ACCOUNT_ID` · `R2_ACCOUNT_ID` | stack `src/index.ts:88-93` | stack `wrangler.toml:156,190-193`; GH repo secret | no — write-only | account selection for Vectorize/D1/R2 | stack Worker (4 aliases), GH, `.env` | account id, not a credential |
| `D1_DATABASE_MAP` | stack `src/index.ts:94`; `digivault/src/digivault/server.py:235` | stack `wrangler.toml:165`; GH repo secret | no — write-only | per-tenant D1 corpus routing | stack Worker + GH | JSON of names→ids |
...
| `DIGIKEY_BLOCKLIST_REDIS_URL` · `DIGIKEY_REQUIRE_BLOCKLIST` | `digikey/src/digikey/blocklist.py:36,41` | `.env`; container env | yes | revoked JTIs stay valid / fail-closed 503 | compose default `1`, `.env.example` default `0` | see [ADR-0007](../adr/0007-digikey-revocation.md) |
...
| `DIGISEARCH_URL` (secret) · `DIGISEARCH_URL` (var) | `docs-reindex-guide.yml:86`; `pipeline-digiquant.yml:155` | GH secret **and** GH var | n/a | reindex workflow / pipeline | same name in two stores | collision; should be a var only |
| `DIGIKEY_ISSUER` · `DIGIKEY_AUDIENCE` · `DIGIKEY_KEY_ID` · `DIGIKEY_JWKS_URL` | stack `src/index.ts:61-62,73`; `jwt_verify.py:60` | stack `wrangler.toml:201-202` | yes | `iss`/`aud` mismatch fails all verification | wrangler + entrypoint defaults | plain vars, not secrets |
...
- **GitHub repo secret/var set not enumerable here.** The audit mapped workflow *references*; the actual set of ~42 repo secrets + 6 vars is not read back. Whether `CORE_SUPABASE_*` exist or workflows silently fall back to legacy names is unverified.
...
- **Revocation strength.** Whether prod sets `DIGIKEY_BLOCKLIST_REDIS_URL` and `DIGIKEY_REQUIRE_BLOCKLIST=1` (local `.env.example` defaults `0`, compose `1`).
...
- **Sibling docs.** `docs/ops/SECRETS_ROTATION.md` and `docs/adr/0029-secrets-management.md` are referenced above as plain code spans, not links: they are not present in this worktree, and a markdown link to a missing path fails `make doc-check`.
