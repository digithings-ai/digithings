...
| Booked positions, missing H9 ledger | Operator recovery: `python digiquant/scripts/research/recover_commit_ledger.py --date YYYY-MM-DD` (then `--apply`). Reads house `positions` / `nav_history`; calls `append_commit_chain`. Do not re-run the LLM pipeline. Do not `workflow_dispatch`. |
