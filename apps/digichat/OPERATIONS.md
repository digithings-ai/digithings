...
Apply new tables with `cd apps/digichat && npm run db:migrate` (see [drizzle/0001_conversations.sql](drizzle/0001_conversations.sql), [drizzle/0002_quant_runs.sql](drizzle/0002_quant_runs.sql) for quant run storage).
...
The old `cloudflare/website/` landing and `website/digichat/` zero-dependency demo were removed. Marketing chat is **`apps/digithings-web` `/chat`**; digichat itself lives in **`apps/digichat/`**.
