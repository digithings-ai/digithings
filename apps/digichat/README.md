...
Live digithings.ai chat is **`apps/digithings-web`** (`app/chat/page.tsx` + `ChatEmbedShell`) → same-origin iframe:
...
cd apps/digichat && npm run dev   # http://127.0.0.1:3000

# Terminal 2 — marketing parent (apps/digithings-web, not cloudflare/website)
cd apps/digithings-web
