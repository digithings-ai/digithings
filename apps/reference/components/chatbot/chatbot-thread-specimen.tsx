 ...
 * `/embed` (`@digithings/ui/chat/thread`) plus a local fixture runtime.
 */
import {
  Thread,
  type ComposerLayout,
  type ThreadComponents,
} from "@digithings/ui/chat/thread";
  ...
  return (
    <section className="section-block" id={composerLayout === "compact" ? "composer-compact" : "thread"}>
      ...
      <p className="section-copy">
        {copy ??
          "The fixture streams reasoning, then real MCP tool rows (website: digisearch_query + digivault_get_note; dashboard: list_strategies + run_backtest), then markdown. Turns use the ChatMessage grammar: both left, cube markers — not bubbles. Copy, regenerate, and attach are the registry actions. Welcome copy sits above the composer; optional example rows come from deploy-shaped config."}
      </p>
      <div
