 ...
 *  model can't be front-run off the tearsheet (#1462). Rendered as the kit
 *  Badge with simple copy ("Signals delayed 3 days" / "Signals +3d
...
export function SignalDelayChip({
  ...
  return (
    <Badge
      variant="outline"
      className={"border-accent-weak bg-accent-weak" + (className ? ` ${className}` : "")}
      title={tip}
      aria-label={tip}
    >
      {label}
