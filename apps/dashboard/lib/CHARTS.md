...
the scaffold itself lives in `@digithings/ui` (the finance-charts family's
...
`SignedBars` from `@digithings/ui`, #1463) and is out of scope for both
...
ruling lives in `packages/ui/CHARTS.md`.
...
> **Historical as of #1747.** Every consuming surface below lived on the
> Performance tab and is deleted. These entries are **kept deliberately**: they
> are the promotion specs digiweb would need to close the gaps, and the record a
> revival would start from. Read them in the past tense.
...
- **`<TagsInput/>` (full field) vs the NAV-comparables picker** (composed
  ...
  (dress `ctl-search-row`) from `@digithings/ui` and keeps the trigger,
...
- **`<PerformanceDashboard/>` ← server-metrics strip** (FULL adoption,
  ...
  `@import "@digithings/ui/styles/finance-composites.css"` and
