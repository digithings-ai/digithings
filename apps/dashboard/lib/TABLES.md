...
> **Ruling: the promoted `<SortableTable/>` leaderboard (`@digithings/ui`
...
> (`packages/ui/MIGRATION.md`, promotion playbook step 3 — the
> `lib/CHARTS.md` engine ruling is the in-app precedent).

## What the primitive offers

`SortableTable` (`packages/ui/src/components/finance-composites/SortableTable.tsx`)
...
uniform column tones) should adopt `<SortableTable/>` from `@digithings/ui`
instead of hand-rolling sort state — wire
`@import "@digithings/ui/styles/finance-composites.css"` (plain, no
...
|---|---|---|---|---|---|
| NAV chart | `public_accounting_nav_history` (`ACCOUNTING_NAV_VIEW`) | Each row `date` | Paper NAV index (not a headline $) | Fail closed on query error (#3029) — never empty-as-healthy | Tip badge from **latest dated row** only: `finalized accounting` vs `legacy estimate`. Mixed history with a legacy tip is **not** finalized. Plotted as one base-100 continuity index: source runs chain each row's own day return (seam basis changes carry flat) and calendar gaps ≤4 days forward-fill the last value (#4014). |
...
| Since inception | Chained base-100 continuity index tip − 100 (#4014) | First NAV date → tip date | Percentage points | Null with fewer than 2 finite NAV points | Same overlay rule as day return. Brief persisted path must match Tearsheet within 0.05 pp when overlay is off. |
...
| Marks / unrealized | Open-book `positions.metrics_as_of` + `current_price` / market-API `GET /v1/market/closes` (#4053, R2-API-only — empty when `NEXT_PUBLIC_MARKET_DATA_URL` is unset, no Supabase fallback) | `metrics_as_of` or close date | Unrealized % vs avg entry | Fail closed without basis or mark | `marksUnstamped` when the open book is empty or any row lacks `metrics_as_of`. Chrome must not imply a refresh. |
...
- `apps/dashboard/lib/performance-ssot.ts` — pure SSOT helpers
- `apps/dashboard/lib/observability-queries.ts` — `getPerformanceBundle()` (Tearsheet + shared builder)
- `apps/dashboard/app/page.tsx` — Brief persisted path + live-marks badge
- `apps/dashboard/app/portfolio/performance/page.tsx` — Tearsheet consumer
