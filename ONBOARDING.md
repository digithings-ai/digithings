...
- **Stray `node_modules/` at the repo root** means you ran `npm install` in the wrong directory. Workspace installs must happen under `frontend/` (design-system workspace root) or a specific app dir.
- **Atlas frontend regeneration**: `apps/digiquant-atlas/frontend/next-env.d.ts` and `tsconfig.json` are rewritten by Next.js / your IDE. Discard those diffs unless the change is deliberate.
