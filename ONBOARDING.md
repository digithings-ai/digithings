...
- **Task branches are worktrees** — `make task ISSUE=N` creates a worktree under `.worktrees/task/N-slug/` (gitignored). Don't `cd` out of it mid-task; close with `git worktree remove` after the PR merges.
